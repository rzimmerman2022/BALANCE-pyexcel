...
def create_sample_parquet(file_path: Path, num_rows: int = 1000) -> pd.DataFrame:
    ...
    data = {
        ...
        "Description": [f"Transaction description {i} with extra details" for i in range(num_rows)],
        ...
        "Notes": [f"Some notes for transaction {i}, could be lengthy." * 5 for i in range(num_rows)]
    }
    df = pd.DataFrame(data)
    # Ensure data types are representative
    df["SplitPercent"] = df["SplitPercent"].astype("float64") # Allow NA if needed, though not here
    df["Amount"] = df["Amount"].astype("float64")
    df["Date"] = pd.to_datetime(df["Date"])
    
    # Ensure string columns are actual strings, not objects if possible, for memory calculation
    for col in ["TxnID", "SharedFlag", "Description", "Category", "Merchant", "Account", "Notes"]:
...
def test_selective_column_load_memory_reduction(tmp_path):
    ...
    original_df = create_sample_parquet(parquet_file, num_rows=10000) # Use a decent number of rows
    ...
    df_selective_load = pd.read_parquet(parquet_file, columns=columns_to_load_selectively)
    ...
    assert all(col in df_full_load.columns for col in columns_to_load_selectively) # Sanity check
    ...
    assert memory_selective_load < memory_full_load, \
    ...
    assert memory_selective_load < (memory_full_load * 0.5), \
    ...
    for col in columns_to_load_selectively:
        ...
        assert pd.api.types.is_dtype_equal(df_selective_load[col].dtype, original_df[col].dtype), \
...
def test_selective_load_handles_missing_columns_gracefully(tmp_path):
    ...
    columns_to_try_load = ["TxnID", "SharedFlag", "SplitPercent"] # "SharedFlag" is missing
    ...
    assert "SharedFlag" in str(excinfo.value).replace("'", "").replace('"', ''), \
