...
def sample_queue_df() -> pd.DataFrame:
    data = {
        ...
        QUEUE_SPLIT_COL: [None, "abc", "60.5", "50", "50"] # TX2 has invalid split, TX5 split is irrelevant
    }
    return pd.DataFrame(data)

def test_sync_empty_transactions():
    df_trans = pd.DataFrame(columns=[TRANS_TXNID_COL, TRANS_SHARED_FLAG_COL, TRANS_SPLIT_PERC_COL])
...
def test_sync_empty_queue():
    ...
    df_queue = pd.DataFrame(columns=[QUEUE_TXNID_COL, QUEUE_DECISION_COL, QUEUE_SPLIT_COL])
...
def test_sync_invalid_decision_value_ignored(caplog):
    df_trans = sample_transactions_df()
    queue_data = {
        QUEUE_TXNID_COL: ["TX1"], QUEUE_DECISION_COL: ["Invalid"], QUEUE_SPLIT_COL: [None]
    }
    df_queue = pd.DataFrame(queue_data)
    result_df = sync_review_decisions(df_trans.copy(), df_queue)
    ...
    assert_frame_equal(result_tx1.reset_index(drop=True), original_tx1.reset_index(drop=True))

def test_sync_split_decision_missing_percent_defaults_to_50(caplog):
    df_trans = sample_transactions_df()
    # TX1: S decision, missing split percent
    queue_data = {
        QUEUE_TXNID_COL: ["TX1"], QUEUE_DECISION_COL: ["S"], QUEUE_SPLIT_COL: [None] # or np.nan
    }
    df_queue = pd.DataFrame(queue_data)
    result_df = sync_review_decisions(df_trans.copy(), df_queue)
    
    assert "Missing split percentage for split decision on TxnID: TX1. Using 50%." in caplog.text
    tx1_res = result_df[result_df[TRANS_TXNID_COL] == "TX1"].iloc[0]
...
def test_sync_split_decision_invalid_percent_format_defaults_to_50(caplog):
    ...
    queue_data = {
        QUEUE_TXNID_COL: ["TX1"], QUEUE_DECISION_COL: ["S"], QUEUE_SPLIT_COL: ["abc"]
    }
    df_queue = pd.DataFrame(queue_data)
    result_df = sync_review_decisions(df_trans.copy(), df_queue)
...
def test_sync_split_percent_out_of_bounds_clamped(caplog):
    ...
    assert "Split percentage (-10.0) below 0 for TxnID: TX1. Clamping to 0." in caplog.text
    tx1_res = result_df[result_df[TRANS_TXNID_COL] == "TX1"].iloc[0]
    assert tx1_res[TRANS_SHARED_FLAG_COL] == "S"
    assert tx1_res[TRANS_SPLIT_PERC_COL] == 0.0

    assert "Split percentage (110.0) above 100 for TxnID: TX2. Clamping to 100." in caplog.text
    tx2_res = result_df[result_df[TRANS_TXNID_COL] == "TX2"].iloc[0]
...
def test_sync_txn_id_not_in_transactions_is_ignored():
    ...
    queue_data = {
        QUEUE_TXNID_COL: ["TX_UNKNOWN"], QUEUE_DECISION_COL: ["Y"], QUEUE_SPLIT_COL: [None]
    }
    df_queue = pd.DataFrame(queue_data)
    result_df = sync_review_decisions(df_trans.copy(), df_queue)
    # Result should be identical to original df_trans as TX_UNKNOWN is not in df_trans
    assert_frame_equal(result_df.reset_index(drop=True), df_trans.reset_index(drop=True))

def test_sync_case_insensitivity_and_whitespace_in_decision():
    df_trans = sample_transactions_df()
    # TX1: decision ' y '
    queue_data = {
        QUEUE_TXNID_COL: ["TX1"], QUEUE_DECISION_COL: [" y "], QUEUE_SPLIT_COL: [None]
    }
    df_queue = pd.DataFrame(queue_data)
    result_df = sync_review_decisions(df_trans.copy(), df_queue)
...
def test_sync_preserves_other_columns():
    ...
    original_desc_amount = original_desc_amount.sort_values(by=TRANS_TXNID_COL).reset_index(drop=True)
    result_desc_amount = result_desc_amount.sort_values(by=TRANS_TXNID_COL).reset_index(drop=True)
...
def test_sync_split_percent_for_non_s_decision_is_ignored():
    ...
    queue_data_tx4 = {
        QUEUE_TXNID_COL: ["TX4"], QUEUE_DECISION_COL: ["N"], QUEUE_SPLIT_COL: ["25"]
    }
...
def test_sync_empty_decision_in_queue_is_ignored():
    ...
    assert_frame_equal(result_tx1_tx2.reset_index(drop=True), original_tx1_tx2.reset_index(drop=True))
