...
PARQUET_FILE_PATH = project_root / "data" / "processed" / "combined_transactions.parquet"
...
def cleanup_parquet_file():
    ...
    try:
        if PARQUET_FILE_PATH.parent.exists() and not any(PARQUET_FILE_PATH.parent.iterdir()):
            PARQUET_FILE_PATH.parent.rmdir()
        if PARQUET_FILE_PATH.parent.parent.exists() and not any(PARQUET_FILE_PATH.parent.parent.iterdir()): # data/
...
def test_append_to_master_parquet(cleanup_parquet_file):
    ...
    try:
        ...
        assert len(result_df.columns) == len(MASTER_COLS), \
            f"Expected {len(MASTER_COLS)} columns, got {len(result_df.columns)}. Columns: {result_df.columns.tolist()}"
        for col in MASTER_COLS:
            assert col in result_df.columns, f"Expected column '{col}' not found in Parquet table."
        ...
        assert pd.api.types.is_datetime64_any_dtype(result_df["TransDate"]), \
        ...
        assert pd.api.types.is_datetime64_any_dtype(result_df["PostDate"]), \
             f"PostDate column is not datetime. Dtype: {result_df['PostDate'].dtype}"


        # Check Owner column content
        assert (result_df["Owner"] == owner_name).all(), "Owner column not correctly populated."

        # Check that missing columns were filled with NA (which become None/NaT in result_df)
        # For PostDate (datetime), it should be NaT
        assert result_df["PostDate"].isna().all(), "PostDate (missing in input) should be all NaT."
        # For ReferenceNumber (object/string), it should be None or NaN (pd.NA becomes float NaN sometimes)
        assert result_df["ReferenceNumber"].isna().all(), "ReferenceNumber (missing in input) should be all NA."
        ...
        assert len(result_df_after_append) == len(dummy_df) + len(dummy_df2), \
