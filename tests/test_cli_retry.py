...
def test_parquet_write_succeeds_on_retry(monkeypatch, caplog):
    ...
    with patch.object(cli.log, "info") as mock_log_info, patch.object(
        cli.log, "warning"
    ) as mock_log_warning, patch.object(cli.log, "error") as mock_log_error:
        for attempt in range(max_retries_in_cli):
            try:
                if attempt == 0:
                    mock_log_info(
                        f"Writing final DataFrame ({len(SAMPLE_DF)} rows) to Parquet: {DUMMY_PARQUET_PATH} (Engine: pyarrow, Compression: zstd)"
                    )
                ...
                if attempt < max_retries_in_cli - 1:
                    ...
                    mock_log_error(
                        f"Failed to write Parquet file {DUMMY_PARQUET_PATH} after {max_retries_in_cli} attempts. Last error: {e}",
                        exc_info=True,
                    )
    ...
    assert any(
        f"Successfully wrote Parquet file on attempt 3/{max_retries_in_cli}"
        in call_args[0][0]
        for call_args in mock_log_info.call_args_list
    )
    assert any(
        "Parquet write attempt 1/5 failed" in call_args[0][0]
        for call_args in mock_log_warning.call_args_list
    )
    assert any(
        "Parquet write attempt 2/5 failed" in call_args[0][0]
        for call_args in mock_log_warning.call_args_list
    )
    assert any(
        "Retrying Parquet write in 0.20 seconds..." in call_args[0][0]
        for call_args in mock_log_info.call_args_list
    )
    assert any(
        "Retrying Parquet write in 0.40 seconds..." in call_args[0][0]
        for call_args in mock_log_info.call_args_list
    )
...
def test_parquet_write_fails_after_all_retries(monkeypatch, caplog):
    ...
    # Check log messages
    assert any(
        f"Failed to write Parquet file {DUMMY_PARQUET_PATH} after {max_retries_in_cli} attempts"
        in call_args[0][0]
        for call_args in mock_log_error.call_args_list
    )
    for i in range(1, max_retries_in_cli + 1):
        assert any(
            f"Parquet write attempt {i}/5 failed" in call_args[0][0]
            for call_args in mock_log_warning.call_args_list
        )
    for i in range(max_retries_in_cli - 1):  # 4 sleep calls
        delay = base_delay_in_cli * (2**i)
        assert any(
            f"Retrying Parquet write in {delay:.2f} seconds..." in call_args[0][0]
            for call_args in mock_log_info.call_args_list
        )
