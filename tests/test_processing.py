...
def test_expense_pipeline_basic_shared_math():
    ...
    for idx, row in shared_rows.iterrows():
        ...
        if payer == 'Ryan':
            ...
            assert abs(jordyn_owes - expected_jordyn_owes) < 0.01, f"Row {idx}: JordynOwes ({jordyn_owes}) != expected ({expected_jordyn_owes})"
        else:
            # Jordyn paid, so Ryan owes his portion to Jordyn
            assert jordyn_owes == 0.0, f"Row {idx}: When Jordyn pays, JordynOwes should be 0"
            expected_ryan_owes = allowed_amount * config.RYAN_PCT
            assert abs(ryan_owes - expected_ryan_owes) < 0.01, f"Row {idx}: RyanOwes ({ryan_owes}) != expected ({expected_ryan_owes})"
    ...
    assert personal_row['JordynOwes'] == 0.0, "Personal expense should have JordynOwes = 0"
    
    # Validate specific calculations for Ryan paying shared expense
    ryan_shared_row = result_df[(result_df['Payer'] == 'Ryan') & (result_df['IsShared'] == True)].iloc[0]
    expected_jordyn_owes = ryan_shared_row['AllowedAmount'] * config.JORDYN_PCT
    assert abs(ryan_shared_row['JordynOwes'] - expected_jordyn_owes) < 0.01, "Ryan paid shared: JordynOwes calculation incorrect"
    
    # Validate specific calculations for Jordyn paying shared expense  
    jordyn_shared_row = result_df[(result_df['Payer'] == 'Jordyn') & (result_df['IsShared'] == True)].iloc[0]
    expected_ryan_owes = jordyn_shared_row['AllowedAmount'] * config.RYAN_PCT
    assert abs(jordyn_shared_row['RyanOwes'] - expected_ryan_owes) < 0.01, "Jordyn paid shared: RyanOwes calculation incorrect"
    
    # Check required columns exist
    required_columns = ['Date', 'ActualAmount', 'AllowedAmount', 'Payer', 'Description', 'Merchant',
