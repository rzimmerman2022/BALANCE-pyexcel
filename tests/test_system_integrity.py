...
class TestSystemIntegrity(unittest.TestCase):
    ...
    def test_rent_allocation(self):
        ...
        jordyn_rent = audit[(audit['person'] == 'Jordyn') & (audit['merchant'] == 'Rent')]
    ...
    def test_standard_50_50_split(self):
        ...
        self.assertAlmostEqual(ryan_exp['net_effect'].iloc[0], -50.00) # Paid 100, owed 50
        self.assertAlmostEqual(jordyn_exp['net_effect'].iloc[0], 50.00)  # Paid 0, owed 50
        self.assertAlmostEqual(summary['net_owed'].sum(), 0.0)
    ...
    def test_full_to_person_rule(self):
        ...
        self.assertAlmostEqual(ryan_ledger['net_effect'].iloc[0], 75.00) # Owed full amount
        self.assertAlmostEqual(jordyn_ledger['net_effect'].iloc[0], -75.00) # Paid full amount
        self.assertAlmostEqual(summary['net_owed'].sum(), 0.0)
    ...
    def test_system_zeros_out(self):
        ...
        expense_data = {'person': ['Ryan'], 'date': ['2025-01-15'], 'merchant': ['Groceries'], 'actual_amount': [100.00]}
        ledger_data = {'person': ['Jordyn'], 'date': ['2025-01-16'], 'merchant': ['Movie Tickets (2x Jordyn)'], 'actual_amount': [50.00]}
        rent_data = {'Month': ['Feb-25'], "Ryan's Rent (43%)": [860], "Jordyn's Rent (57%)": [1140], "Gross Total": [2000]}
