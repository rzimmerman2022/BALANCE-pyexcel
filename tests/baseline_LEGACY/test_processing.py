...
class TestBuildBaseline:
    ...
    def test_zero_sum_balance(self):
        ...
        if not result.empty:
            ...
            assert (
                abs(total_balance) < 100.0
            ), f"Total balance should be reasonable, got ${total_balance:.2f}"
    ...
    def test_lineage_not_null(self):
        ...
        if not result.empty:
            assert (
                result["lineage"].notna().all()
            ), "All rows should have non-null lineage"
            assert (
                result["lineage"] != ""
            ).all(), "All rows should have non-empty lineage"
    ...
    def test_debug_mode_creates_snapshots(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = build_baseline(
                debug=True, inputs_dir=str(fixtures_dir), snapshot_dir=str(snapshot_dir)
            )
    ...
    def test_reconciliation_invariants(self):
        ...
        if not result.empty:
            ...
            ), f"Unexpected persons: {persons - expected_persons}"

            # Test that net_owed values are numeric
            assert result["net_owed"].dtype in [
                int,
                float,
            ], "net_owed should be numeric"
    ...
    def test_lineage_traceability(self):
        ...
        if not result.empty:
            for _, row in result.iterrows():
                ...
                assert (
                    "|" in lineage or len(lineage.split("_")) >= 2
...
class TestCleanerStats:
    ...
    def test_cleaner_stats_creation(self):
        ...
        stats = CleanerStats(
            rows_in=100, rows_out=95, duplicates_dropped=3, bad_dates=2
        )
...
class TestLineageUtils:
    ...
    def test_add_step_id_append(self):
        ...
        df = pd.DataFrame({"col1": [1, 2, 3], "lineage": ["step1", "step1", "step1"]})
...
class TestIntegration:
    ...
    def test_end_to_end_with_real_fixtures(self):
        ...
        total_balance = result["net_owed"].sum()
        assert (
            abs(total_balance) < 1000
        ), f"Total balance seems unreasonable: ${total_balance:.2f}"
    ...
    def test_empty_source_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (temp_path / "expenses.csv").write_text(
                "Name,Date of Purchase,Actual Amount\n"
            )
            (temp_path / "ledger.csv").write_text(
                "Name,Date of Purchase,Actual Amount\n"
            )
...
class TestPerformance:
    ...
    def test_debug_mode_performance(self, debug_mode):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            start_time = time.time()
            build_baseline(
                debug=debug_mode, inputs_dir=str(fixtures_dir), snapshot_dir=temp_dir
            )
            duration = time.time() - start_time
            ...
            assert (
                duration < max_time
            ), f"Execution took {duration:.2f}s with debug={debug_mode}"
