...
def analyze_expense_structure(df):
    ...
    analysis['date_range'] = f"{df['Date'].min()} to {df['Date'].max()}" if 'Date' in df.columns else "Unknown"
...
def analyze_shared_vs_all_expenses(df):
    ...
    if 'Name' in df.columns and 'Actual Amount' in df.columns:
        ...
        print(f"  Sum of balances: ${ryan_balance_wrong + jordyn_balance_wrong:,.2f} (SHOULD BE ZERO!)")
    ...
    if 'Allowed Amount' in df.columns:
        ...
        shared_jordyn = shared_only[shared_only['Name'] == 'Jordyn']['Allowed Amount'].sum()
...
def detect_settlements(df):
    ...
    settlement_keywords = ['venmo', 'zelle', 'paypal', 'cash app', 'settlement', 'payment to', 'payment from']
        ...
        for name in ['Ryan', 'Jordyn']:
            ...
            if mask.any():
                ...
                for _, row in potential_settlements.iterrows():
                    ...
                    if not any(s['date'] == row.get('Date') and s['amount'] == row.get('Actual Amount', 0) for s in settlements):
                        settlements.append({
                            'date': row.get('Date', 'Unknown'),
                            'payer': row.get('Name', 'Unknown'),
                            'amount': row.get('Actual Amount', 0),
    ...
    for s in settlements[:5]:  # Show first 5
        print(f"  {s['date']}: {s['payer']} - ${s['amount']:.2f} - {s['description'][:50]}")
...
def calculate_correct_balance(expenses_df, rent_df):
    ...
    balance['final_balance']['ryan'] = balance['expense_balance']['ryan'] + balance['rent_balance']['ryan']
    balance['final_balance']['jordyn'] = balance['expense_balance']['jordyn'] + balance['rent_balance']['jordyn']
    
    print("\nFINAL CORRECT BALANCE CALCULATION:")
    print("-"*40)
    print(f"From expenses: Ryan {'owes' if balance['expense_balance']['ryan'] > 0 else 'is owed'} ${abs(balance['expense_balance']['ryan']):,.2f}")
    ...
    print(f"              Jordyn {'owes' if balance['final_balance']['jordyn'] > 0 else 'is owed'} ${abs(balance['final_balance']['jordyn']):,.2f}")
...
def create_analysis_visualizations(expenses_df, rent_df, balance_comparison):
    ...
    try:
        ...
        if 'Allowed Amount' in expenses_df.columns:
            ax = axes[0, 0]
            shared = expenses_df[expenses_df['Allowed Amount'] > 0]['Allowed Amount'].sum()
            personal = expenses_df[expenses_df['Allowed Amount'] == 0]['Actual Amount'].sum()
            
            ax.pie([shared, personal], labels=['Shared', 'Personal'], autopct='%1.1f%%', startangle=90)
        ...
        if 'wrong_method' in balance_comparison and 'correct_method' in balance_comparison:
            ax = axes[1, 1]
            methods = ['Wrong\n(All Expenses)', 'Correct\n(Shared Only)']
            totals = [balance_comparison['wrong_method']['total'], 
...
def generate_recommendations(results):
    ...
    if 'settlements' in results['balance_analysis'] and results['balance_analysis']['settlements']:
        recommendations.append(
    ...
    if 'data_quality' in results and 'expenses' in results['data_quality']:
        ...
        if expense_quality.get('personal_count', 0) > expense_quality.get('shared_count', 0):
            recommendations.append(
