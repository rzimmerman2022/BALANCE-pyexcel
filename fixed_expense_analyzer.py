...
class EnhancedSharedExpenseAnalyzer:
    ...
    def _load_and_clean_expense_data(self) -> pd.DataFrame:
        ...
        explicitly_personal = df['AllowedAmount'].astype(str).str.strip().str.match(r'^\$\s*-\s*$', na=False)
    ...
    def _perform_advanced_analytics(self, master_ledger: pd.DataFrame) -> Dict[str, Any]:
        ...
        for idx, transaction_date in enumerate(master_ledger['Date']):
            ...
            if current_balance_value > self.config.LIQUIDITY_STRAIN_THRESHOLD:
                ...
                elif not future_transactions.empty:
    ...
    def _generate_recommendations(self, analytics: Dict[str, Any], 
        ...
        if risk_assessment['financial_risks']['concentration_risk']['rent_percentage'] > 30:
            recommendations['strategic_changes']['medium_impact_low_effort'].append({
                ...
                'financial_impact': reconciliation['total_shared'] * 0.3,
                'timeline': '30 days',
        ...
        recommendations['strategic_changes']['medium_impact_high_effort'].append({
            ...
            'financial_impact': analytics['statistical_summary']['outlier_transactions'] * 1000,
            'timeline': '30 days',
    ...
    def _categorize_merchant(self, merchant: str) -> str:
        ...
        categories = {
            ...
            'Shopping': ['amazon', 'best buy', 'mall', 'store', 'cb & cb']
        }
    ...
    def _validate_results(self, reconciliation: Dict[str, Any], 
        ...
        # 3. No negative balances for individual transactions (only check shared)
        shared_transactions = master_ledger[master_ledger['IsShared']]
        self.validation_results['no_negative_transactions'] = not any(
            shared_transactions['AllowedAmount'] < 0
        )
    ...
    def _generate_outputs(self, master_ledger: pd.DataFrame, reconciliation: Dict[str, Any],
        ...
        pdf_content = self._generate_pdf_report(master_ledger, reconciliation, analytics, risk_assessment, recommendations)
    ...
    def _generate_pdf_report(self, master_ledger: pd.DataFrame, reconciliation: Dict[str, Any], analytics: Dict[str, Any],
        ...
        report.append(f"Data Quality Score: {self._calculate_data_quality_score(master_ledger):,.1f}%")
        ...
        report.append("-" * 50)
        report.append(f"\nTotal Shared Expenses: ${reconciliation['total_shared']:,.2f}")
        report.append(f"Ryan's Fair Share (43%): ${reconciliation['ryan_fair_share']:,.2f}")
        report.append(f"Jordyn's Fair Share (57%): ${reconciliation['jordyn_fair_share']:,.2f}")
        report.append(f"\nRyan Actually Paid: ${reconciliation['ryan_paid']:,.2f}")
        report.append(f"Jordyn Actually Paid: ${reconciliation['jordyn_paid']:,.2f}")
        report.append(f"\nRyan's Variance: ${reconciliation['ryan_variance']:,.2f}")
        report.append(f"Jordyn's Variance: ${reconciliation['jordyn_variance']:,.2f}")
        
        # Category Breakdown
        report.append("\n3. EXPENSE CATEGORIES")
        report.append("-" * 50)
        for cat_detail in reconciliation['category_details']:
            report.append(f"\n{cat_detail['Category']}:")
            report.append(f"  Total: ${cat_detail['Total']:,.2f}")
            report.append(f"  Ryan Paid: ${cat_detail['RyanPaid']:,.2f}")
            report.append(f"  Jordyn Paid: ${cat_detail['JordynPaid']:,.2f}")
        
        # Risk Assessment
        report.append("\n4. RISK ASSESSMENT")
        report.append("-" * 50)
        report.append(f"\nOverall Risk Score: {risk_assessment['overall_assessment']['risk_score']:.1f}/100")
        report.append(f"Risk Level: {risk_assessment['overall_assessment']['risk_level']}")
        report.append(f"\nHigh Risks: {risk_assessment['overall_assessment']['high_risk_count']}")
        report.append(f"Medium Risks: {risk_assessment['overall_assessment']['medium_risk_count']}")
        report.append(f"Low Risks: {risk_assessment['overall_assessment']['low_risk_count']}")
        
        # Data Quality
        report.append("\n5. DATA QUALITY REPORT")
        report.append("-" * 50)
        report.append(f"\nData Quality Score: {self._calculate_data_quality_score(master_ledger):,.1f}%")
        report.append(f"Total Transactions: {len(master_ledger)}")
        report.append(f"Clean Transactions: {(master_ledger['DataQualityFlag'] == DataQualityFlag.CLEAN.value).sum()}")
        report.append(f"Issues Found: {len(self.data_quality_issues)}")
