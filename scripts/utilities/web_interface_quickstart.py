...
def display_dashboard(df):
    ...
    with col3:
        amount_filter = st.slider(
            ...
            value=(float(df['amount'].min()), float(df['amount'].max())) if 'amount' in df.columns else (0, 1000)
        )
    ...
    if search_term:
        search_cols = ['description', 'merchant', 'category'] if 'description' in df.columns else df.columns
...
def main():
    ...
    # Sidebar
    with st.sidebar:
        st.image("https://via.placeholder.com/300x100/1e88e5/ffffff?text=BALANCE", width=300)
        st.markdown("---")
        ...
        if st.session_state.processing_history:
            ...
            for item in st.session_state.processing_history[-5:]:
                st.text(f"• {item['timestamp'].strftime('%H:%M')} - {item['files']} files")
    ...
    if st.session_state.current_tab == "Upload & Process":
        st.markdown('<h1 class="main-header">📤 Upload & Process CSV Files</h1>', unsafe_allow_html=True)
        ...
        if st.session_state.processed_data is not None:
            ...
            st.warning("⚠️ No data available. Please upload and process CSV files first.")
        ...
        if st.session_state.processed_data is not None:
            ...
            if 'category' in df.columns:
                ...
                category_spending = df[df['amount'] < 0].groupby('category')['amount'].sum().abs().sort_values(ascending=False).head(10)
            ...
            if 'merchant' in df.columns or 'description' in df.columns:
                ...
                top_merchants = df[df['amount'] < 0].groupby(merchant_col)['amount'].agg(['sum', 'count'])
                top_merchants['sum'] = top_merchants['sum'].abs()
                top_merchants = top_merchants.sort_values('sum', ascending=False).head(15)
            ...
            st.warning("⚠️ No data available for analytics. Please upload and process CSV files first.")
        ...
        auto_categorize = st.checkbox("Automatic transaction categorization", value=True)
