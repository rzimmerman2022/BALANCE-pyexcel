...
def create_dedup_key(row):
    ...
    date_key = (
        row["date"].strftime("%Y-%m-%d") if pd.notnull(row["date"]) else "unknown"
    )
    ...
    for key, group in df.groupby("dedup_key"):
        if len(group) == 1:
            ...
            for _, row in group.iterrows():
                ...
                info_score = 0
                if row["merchant"] and str(row["merchant"]).strip() not in [
                    "",
                    "Unknown",
                ]:
                    info_score += 2
                ...
                if row["institution"] and str(row["institution"]).strip():
                    info_score += 1
                if row["category"] and str(row["category"]).strip() not in [
                    "",
                    "Uncategorized",
                ]:
                    info_score += 1
...
def main():
    ...
    combined_df["merchant_standardized"] = combined_df.apply(
        lambda x: standardize_merchant(x["description"], x["merchant"]), axis=1
    )
    ...
    combined_df["potential_refund"] = combined_df["description"].str.contains(
        r"(?i)(refund|return|reversal|dispute|chargeback|credit.*adjustment)", na=False
    )
    ...
    try:
        ...
        print(
            f"Note: Parquet export failed ({e}), but CSV and Excel files are available"
        )
