...
def find_refunds_for_merchant(df, merchant_name, days_back=90):
    ...
    merchant_mask = df["merchant_standardized"].str.contains(
    ...
    if len(results) > 0:
        ...
        print(
            results[
                ["date", "merchant_standardized", "amount", "description"]
            ].to_string()
        )
        print(
...
def find_duplicate_charges(df, days_window=3):
    ...
    for (merchant, amount), group in df[df["amount"] < 0].groupby(
        ["merchant_standardized", "amount_abs"]
    ):
        if len(group) > 1:
            ...
            for i in range(len(group_sorted) - 1):
                date_diff = (
                    group_sorted.iloc[i + 1]["date"] - group_sorted.iloc[i]["date"]
                ).days
...
def check_refund_status(df, merchant, charge_amount, charge_date_str):
    ...
    amount_mask = (
        abs(df["amount"] - abs(charge_amount)) < 0.01
    )  # Allow penny difference
    ...
    if len(potential_refunds) > 0:
        ...
        print("✗ No refund found within 60 days of charge")
...
def analyze_disputes(df):
    ...
    if total_disputes > 0:
        ...
        print(
            f"Date range: {dispute_df['date'].min().date()} to {dispute_df['date'].max().date()}"
        )

        print("\n=== Top Merchants with Disputes ===")
        merchant_disputes = (
            dispute_df.groupby("merchant_standardized")
            .agg({"amount": ["count", "sum"]})
            .round(2)
        )
        merchant_disputes.columns = ["Count", "Total Amount"]
        print(
            merchant_disputes.sort_values("Count", ascending=False).head(10).to_string()
        )
...
def interactive_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            filename = (
                f"disputes_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            )
            ...
            try:
                ...
                print(
                    results[["date", "merchant_standardized", "amount", "description"]]
                    .head(20)
                    .to_string()
                )
                ...
                if export == "y":
                    filename = (
                        f"custom_search_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                    )
