...
def find_transaction_table(tables, pdf_name: str) -> pd.DataFrame | None:
    ...
    """
    transaction_df = None
    expected_headers = ["trans", "post", "description", "charges", "credits", "amount", "date", "details", "merchant"] # Expanded keywords

    log.debug(f"Searching for transaction table among {tables.n} extracted tables for {pdf_name}...")
    best_overall_score = -1
    best_table_index = -1
    best_header_row_index = -1 # Index of the row identified as header within the best table

    for i, table in enumerate(tables):
        if not hasattr(table, 'df') or table.df.empty or table.df.shape[1] < 4: # Need at least a few columns
            log.debug(f"Table {i} has no valid/sufficient DataFrame, skipping.")
            continue

        df = table.df
        max_rows_to_check = min(5, df.shape[0]) # Check first 5 rows or fewer if table is small
        table_best_score = -1
        table_best_header_row = -1

        # Check original headers first
        cleaned_columns = [str(col).lower().strip().replace('\n', ' ') for col in df.columns]
        header_score = sum(1 for keyword in expected_headers if any(keyword in col for col in cleaned_columns))
        log.debug(f"Table {i} Original Header Score: {header_score}, Columns: {cleaned_columns}")
        ...
        for row_idx in range(max_rows_to_check):
            ...
            if len(row_data) != df.shape[1]:
                log.debug(f"Table {i}, Row {row_idx}: Skipping due to length mismatch ({len(row_data)} vs {df.shape[1]})")
                continue

            cleaned_row = [str(cell).lower().strip().replace('\n', ' ') for cell in row_data]
            row_score = sum(1 for keyword in expected_headers if any(keyword in cell for cell in cleaned_row))
            log.debug(f"Table {i}, Row {row_idx} Score: {row_score}, Data: {cleaned_row}")
        ...
        log.debug(f"Table {i} Best Score: {table_best_score}, Best Header Row Index: {table_best_header_row}")
    ...
    if best_table_index != -1:
        log.info(f"Table {best_table_index} identified as the most likely transaction table (score: {best_overall_score}, header row: {best_header_row_index}).")
        ...
        if best_header_row_index != -1:
            log.warning(f"Promoting row {best_header_row_index} of Table {best_table_index} to header.")
            ...
            transaction_df = transaction_df.iloc[best_header_row_index + 1:].reset_index(drop=True)
        # else: log.debug("Using original headers for the selected table.") # Optional: log if original headers were best

        return transaction_df
    else:
        log.error(f"Could not identify a suitable transaction table in {pdf_name} (max score < 3).")
        return None
...
def process_pdf(pdf_path: Path, owner_output_dir: Path, owner_name: str) -> bool:
    ...
    try: # Wrap main logic in a try block
        ...
        try:
            ...
            stream_tables = camelot.read_pdf(str(pdf_path), pages="1-end", flavor="stream", suppress_stdout=True, edge_tol=500, row_tol=10)
            ...
            if transaction_df_stream is not None:
                ...
                log.warning(f"Stream mode did not identify a transaction table for {pdf_path.name}.")

        except Exception as e_stream:
            log.warning(f"Camelot stream extraction failed for {pdf_path.name}: {e_stream}. Trying lattice.")
        ...
        if extraction_mode is None:
            try:
                ...
                lattice_tables = camelot.read_pdf(str(pdf_path), pages="1-end", flavor="lattice", suppress_stdout=True, line_scale=40)
                ...
                if transaction_df_lattice is not None:
                    ...
                    log.error(f"Lattice mode also did not identify a transaction table for {pdf_path.name}. Skipping.")
                    return False

            except Exception as e_lattice:
                log.error(f"Camelot lattice extraction also failed for {pdf_path.name}: {e_lattice}. Skipping.")
                return False
        ...
        combined_df.columns = [str(col).lower().strip().replace('\n', ' ') for col in original_columns]
        ...
        desc_col_idx = find_col_index(cleaned_columns, ["description", "merchant", "details"])
        ...
        charges_col_idx = find_col_index(cleaned_columns, ["charges", "debit", "withdrawals"])
        credits_col_idx = find_col_index(cleaned_columns, ["credits", "deposit"])
        amount_col_idx = find_col_index(cleaned_columns, ["amount"]) # For cases where it's a single column
        ...
        charges_col_name = cleaned_columns[charges_col_idx] if charges_col_idx is not None else None
        credits_col_name = cleaned_columns[credits_col_idx] if credits_col_idx is not None else None
        single_amount_col_name = cleaned_columns[amount_col_idx] if amount_col_idx is not None else None


        if not has_charges_credits and not has_single_amount:
            log.error(f"Could not identify Amount columns (neither Charges/Credits nor Amount) in {pdf_path.name}. Columns: {cleaned_columns}. Skipping.")
            return False

        # Select only the columns we intend to use
        cols_to_keep_original = list(final_col_map.values()) # Get original names mapped so far
        if has_charges_credits:
            ...
            final_col_map["Amount"] = single_amount_col_name # Map directly if single column

        # Ensure no duplicates and preserve order
        seen = set()
        cols_to_keep = []
        for col in combined_df.columns: # Iterate through original cleaned columns to keep order
        ...
        rename_dict = {v: k for k, v in final_col_map.items() if v in combined_df.columns} # Ensure col exists
        ...
        for col in cols_to_keep: # Use the list of columns kept before renaming
            ...
            try:
                is_money = combined_df[col].astype(str).str.strip().head(20).str.match(money_re).any()
                if pd.notna(is_money) and is_money:
                    ...
                    combined_df[col] = combined_df[col].astype(str).str.replace(r'[$,]', '', regex=True).str.strip()
                    combined_df[col] = combined_df[col].str.replace(r'\(([\d,]+\.?\d*)\)', r'-\1', regex=True) # (1,234.56) -> -1234.56
                    # Convert to numeric, coercing errors
                    combined_df[col] = pd.to_numeric(combined_df[col], errors='coerce')
            except Exception as e:
                log.warning(f"Error processing column '{col}' during numeric check/clean: {e}")
        ...
        # Get the series
        creds_series = combined_df.get(creds_final_name) if creds_final_name in combined_df else None
        chgs_series = combined_df.get(chgs_final_name) if chgs_final_name in combined_df else None
        tot_series = combined_df.get(tot_final_name) if tot_final_name in combined_df else None
        ...
        if creds_numeric is not None or chgs_numeric is not None:
            log.debug(f"Calculating Amount from Credits ('{creds_final_name}') and/or Charges ('{chgs_final_name}')")
            ...
            log.warning("Could not find standard 'credits'/'charges'/'amount'. Using first identified numeric column as Amount.")
            numeric_cols_final_names = [rename_dict.get(orig_name, orig_name) for orig_name in numeric_cols_original_names if rename_dict.get(orig_name, orig_name) in combined_df]
            ...
            if numeric_cols_final_names:
                ...
                if first_numeric_col != "Amount":
                    ...
                    cols_to_drop_after_calc.extend([col for col in numeric_cols_final_names]) # Drop all including the one used
                else:
                     # Already named 'Amount', just ensure type
                     combined_df["Amount"] = combined_df["Amount"].astype(float)
                     # Drop other numeric columns
                     cols_to_drop_after_calc.extend([col for col in numeric_cols_final_names if col != "Amount"])
            else: # Ensure this else aligns with the 'if numeric_cols_final_names:'
                log.error(f"No numeric columns found to use as fallback Amount for {pdf_path.name}. Skipping.")
        ...
        cols_to_drop_final = [col for col in cols_to_drop_after_calc if col in combined_df.columns and col != "Amount"] # Don't drop the final Amount!
        ...
        if "Amount" not in combined_df.columns:
            log.error(f"'Amount' column not found after processing for {pdf_path.name}. Columns: {combined_df.columns.tolist()}. Skipping.")
            return False
        ...
        combined_df["TransDate"] = combined_df["TransDate"].astype(str).str.split('\n').str[0].str.strip()
        log.debug(f"Cleaned TransDate column head:\n{combined_df['TransDate'].head().to_string()}")
        ...
        combined_df["TransDate"] = pd.to_datetime(combined_df["TransDate"], errors='coerce')
        ...
        if len(combined_df) < initial_rows_date:
            log.warning(f"Dropped {initial_rows_date - len(combined_df)} rows with invalid TransDate.")
        if combined_df.empty:
        ...
        schema_required_cols = ["TransDate", "RawMerchant", "Amount"] # From jordyn_pdf schema map
        if not all(col in final_columns_present for col in schema_required_cols):
             log.error(f"Schema-required columns ({schema_required_cols}) missing before save for {pdf_path.name}. Columns present: {final_columns_present}. Skipping.")
             return False

        # --- ADD NEWLINE CLEANING STEP ---
        log.debug("Cleaning newline characters from string columns before saving...")
        string_cols_to_clean = ["RawMerchant", "ReferenceNumber", "AccountLast4", "PostDate"] # Add others if needed
        for col in string_cols_to_clean:
            if col in combined_df.columns:
                ...
                combined_df[col] = combined_df[col].astype(str).str.replace('nan', '', regex=False).str.replace('NaT', '', regex=False).str.replace('\r', ' ', regex=False).str.replace('\n', ' ', regex=False).str.strip()
                log.debug(f"Cleaned newlines/CRs in column: {col}")
        # --- END NEWLINE CLEANING STEP ---

        final_df = combined_df[final_columns_present] # Keep all successfully processed columns
        log.info(f"Final DataFrame shape {final_df.shape} with columns {final_columns_present}")
