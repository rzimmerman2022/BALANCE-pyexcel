...
_PATTERNS: dict[str, re.Pattern] = {
    k: re.compile(v, flags=re.I) for k, v in _CFG.patterns.items()
}
...
def _detect_patterns(desc: str, payer: str) -> tuple[list[str], tuple[str, str | None]]:
    ...
    if "2x" in desc_l.lower():
        if m := re.search(r"\b2x\s+(Ryan|Jordyn)", desc_l, re.I) or re.search(
            r"(Ryan|Jordyn).*?\b2x\b", desc_l, re.I
        ):
    ...
    for key in (
        ...
        if key in _PATTERNS and _PATTERNS[key].search(desc_l):
            ...
            if key in ("gift", "gift_or_present"):
                return [key], (
                    "full_to",
                    "Jordyn" if payer.lower() == "ryan" else "Ryan",
                )
...
def _apply_split_rules(
    actual: float, rule: tuple[str, str | None], payer: str
) -> tuple[float, float, str]:
    kind, target = rule
    if kind == "standard":
        return actual / 2, actual / 2, "SR | Standard 50/50 split"
    if kind == "double_charge":
        return actual / 2, actual / 2, "DC | Double charge documented"
    if kind == "transfer":
        return (
            (-actual, actual, "TR | Zelle to Ryan")
            if (target or "").title() == "Ryan"
            else (actual, -actual, "TR | Zelle to Jordyn")
        )
    if kind == "zero_out":
        return 0.0, 0.0, "CB | Cash-back"
    who = (target or payer).title()
    return (
        (actual, 0.0, "FT | Full to Ryan")
        if who == "Ryan"
        else (0.0, actual, "FT | Full to Jordyn")
    )
...
def _clean_data(df: pd.DataFrame, file_type: str) -> pd.DataFrame:
    ...
    alias_map = {
        alias.lower(): canon
        for canon, aliases in _CFG.person_aliases.items()
        for alias in aliases
    }
    df["person"] = (
        df["person"].str.strip().str.lower().map(alias_map).fillna(df["person"])
    )

    money_cols = [
        col for col in df.columns if "amount" in col or "rent" in col or "total" in col
    ]
...
def build_baseline(
    df: pd.DataFrame, output_dir: str = "audit_reports"
    ...
    for _, row in df.iterrows():
        ...
        if allowed_amount > 0 and actual_paid == 0:
            ...
            if payer == "Ryan":
                ...
                net_effect_jordyn = (
                    allowed_amount  # Jordyn gets positive allowed amount
                )
            ...
            if rule[0] == "zero_out":
                ...
                allowed_ryan, allowed_jordyn, note = _apply_split_rules(
                    actual_paid, rule, payer
                )
                net_effect_ryan = round(
                    allowed_ryan - (actual_paid if payer == "Ryan" else 0.0), 2
                )
                net_effect_jordyn = round(
                    allowed_jordyn - (actual_paid if payer == "Jordyn" else 0.0), 2
                )

        # Preserve full original description for context
        full_description = (
            f"{row.get('description', '')} | {row.get('merchant', '')}".strip(" |")
        )
        ...
        rows.append(
            {
                ...
                "allowed_amount": allowed_ryan,
                "net_effect": net_effect_ryan,
                "pattern_flags": flags,
        ...
        rows.append(
            {
                ...
                "allowed_amount": allowed_jordyn,
                "net_effect": net_effect_jordyn,
                "pattern_flags": flags,
                "calculation_notes": note,
                "transaction_type": "standard",
                "source_file": row["source_file"],
            ...
            rows.append(
                {
                    ...
                    "actual_amount": full_rent,
                    "allowed_amount": ryan_share,
                    "net_effect": round(ryan_share - full_rent, 2),
                    "pattern_flags": ["rent"],
                    "calculation_notes": "Rent | 43/57 Split (Ryan Pays)",
            ...
            rows.append(
                {
                    ...
                    "pattern_flags": ["rent"],
                    "calculation_notes": "Rent | History",
                    "transaction_type": "rent",
            ...
            rows.append(
                {
                    ...
                    "full_description": rent_hist_description,
                    "actual_amount": 0,
                    "allowed_amount": r["actual_amount"] - r["allowed_amount"],
    ...
    audit_df = audit_df.sort_values(["date", "source_file", "person"]).reset_index(
        drop=True
    )
    ...
    # Final summary
    summary_df = (
        audit_df.groupby("person")["net_effect"]
        .sum()
        .reset_index()
    ...
    if abs(imbalance) > _CFG.rounding_tolerance:
        log.warning(
            f"Net imbalance {imbalance:,.2f} exceeds tolerance ({_CFG.rounding_tolerance})."
        )
