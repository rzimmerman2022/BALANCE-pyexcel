...
# Add this to your config.py file after the existing configuration

# ==============================================================================
# 6. SCHEMA MODE CONFIGURATION
# ==============================================================================

# --- Schema Enforcement Mode ---
# Controls how strictly the pipeline enforces the full 25-column schema.
# "strict" mode: All 25 columns from MASTER_SCHEMA_COLUMNS are always present
#                (current behavior - ensures backward compatibility)
# "flexible" mode: Only core required columns are guaranteed, source-specific
#                  columns are included only when they contain data
SCHEMA_MODE = os.getenv("SCHEMA_MODE", "strict").lower()

# Validate the schema mode
if SCHEMA_MODE not in ["strict", "flexible"]:
    logging.warning(
        f"Invalid SCHEMA_MODE '{SCHEMA_MODE}' specified. Defaulting to 'strict'."
    )
    SCHEMA_MODE = "strict"

logging.info(f"Schema Mode configured to: {SCHEMA_MODE}")
