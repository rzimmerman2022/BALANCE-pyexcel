...
def _explode_audit(audit_note: str) -> Tuple[str, str, str, str]:
    ...
    quality_match = re.search(r"DataQuality: ([\w,]+)", audit_str) # Allow comma in quality flags
    data_quality = quality_match.group(1) if quality_match else DataQualityFlag.CLEAN.value
...
def create_master_ledger(
    ...
    logger_instance.info("Creating master ledger from processed rent and expense data...")

    common_cols = [
        "Date", "TransactionType", "Payer", "Description", "ActualAmount", 
        "AllowedAmount", "IsShared", "RyanOwes", "JordynOwes", "BalanceImpact", 
        "AuditNote", "DataQualityFlag", "Merchant"
    ]
    ...
    for df_iter, name in [(rent_df_c, "processed_rent_df"), (expense_df_c, "processed_expense_df")]:
        if df_iter.empty:
            logger_instance.warning(f"{name} is empty. It will not contribute to master ledger.")
            ...
            if name == "processed_rent_df": rent_df_c = pd.DataFrame(columns=common_cols)
            if name == "processed_expense_df": expense_df_c = pd.DataFrame(columns=common_cols)
            continue # Skip further column checks for this empty df

        for col in common_cols:
            if col not in df_iter.columns:
                logger_instance.warning(f"Column '{col}' missing in {name}. Adding as default.")
    ...
    if not dfs_to_concat:
        logger_instance.error("Both processed rent and expense DataFrames are effectively empty. Master ledger cannot be created.")
        # Define all expected columns for an empty master ledger
        all_master_ledger_cols = common_cols + [
            "RunningBalance", "TransactionID", "DataLineage", 
    ...
    if "Date" in master.columns:
        ...
        logger_instance.error("Master ledger is missing 'Date' column after concat, this is unexpected.")
    ...
    for col in numeric_cols:
        if col in master.columns:
            ...
            master[col] = 0.0
            logger_instance.error(f"Numeric column '{col}' unexpectedly missing in master ledger. Defaulting to 0.")


    master["RunningBalance"] = master["BalanceImpact"].cumsum().round(config.CURRENCY_PRECISION)
    master["TransactionID"] = master.apply(lambda row: _generate_transaction_id(row, date_col="Date"), axis=1)
    ...
    if "DataQualityFlag" not in master.columns: # Should be present from processing step
    ...
    if not master.empty and master["Date"].notna().any():
        logger_instance.info(f"Date range: {master['Date'].min()} to {master['Date'].max()}")
