...
def build_running_balance_timeline(
    ...
    if ledger_df.empty:
        ...
        return (
            output_dir / "placeholder_no_data.png",
            "No data for running balance timeline.",
        )

    fig, ax = plt.subplots(figsize=(12, 6))
    ledger_df.plot(
        x="Date", y="RunningBalance", ax=ax, legend=None, color=TABLEAU_COLORBLIND_10[0]
    )
...
def build_waterfall_category_impact(
    ...
    ledger_df_c["MerchantCategory"] = ledger_df_c.apply(
        ...
        else _categorize_merchant(
            row.get("Merchant", "Other"),
    ...
    category_impacts = (
        ledger_df_c.groupby("MerchantCategory")["BalanceImpact"]
        .sum()
    ...
    if category_impacts.empty:
        ...
        return (
            output_dir / "placeholder_no_data.html",
            "No category impacts for waterfall.",
        )
...
def build_monthly_shared_trend(
    ...
    if len(monthly_shared_series) < 3:
        ...
        return (
            output_dir / "placeholder_no_data.png",
            "Insufficient data for trend plot.",
        )
    ...
    ax.bar(
        x_pos, monthly_shared_series.values, color=TABLEAU_COLORBLIND_10[2], alpha=0.8
    )
...
def build_payer_type_heatmap(
    ...
    if (
        ...
        return (
            output_dir / "placeholder_no_data.html",
            "No data for payer-type heatmap.",
        )
...
def build_calendar_heatmaps(
    ledger_df: pd.DataFrame, output_dir: Path, logger_instance: logging.Logger = logger
    ...
    if shared_only.empty:
        logger_instance.warning(
            "No shared transactions with valid dates for calendar heatmaps."
        )
        return calendar_paths
    ...
    for year_month_period in shared_only["YearMonth"].unique():
        ...
        all_days_in_month = pd.Series(
            index=pd.date_range(start=start_date, end=end_date, freq="D"), dtype=float
        ).fillna(0)
        ...
        ax.set_title(
            f"Daily Shared Spending - {year_month_str}", fontsize=14, fontweight="bold"
        )
...
def build_treemap_shared_spending(
    ...
    if shared_df.empty:
        ...
        return (
            output_dir / "placeholder_no_data.html",
            "No shared transactions for treemap.",
        )
...
def build_anomaly_scatter(
    ...
    if (
        ...
        return (
            output_dir / "placeholder_no_data.png",
            "No data for anomaly scatter plot.",
        )
    ...
    if not plot_df.empty and "BalanceImpact" in plot_df.columns:
        ...
        if len(valid_impacts) > 0:  # Check if there are any non-NaN absolute impacts
            ...
            if (
                len(valid_impacts) > (100 / (100 - threshold_pct))
            ):  # e.g. for 99th percentile, need > 100 points. Simplified: need some points.
...
def build_pareto_concentration(
    ...
    ax2.set_ylim(0, 105)
    ax1.set_title(
        "Pareto Analysis of Shared Spending by Category", fontsize=14, fontweight="bold"
    )
...
def build_sankey_settlements(
    ...
    ryan_paid_shared = shared_expenses[shared_expenses["Payer"].str.lower() == "ryan"][
        "AllowedAmount"
    ].sum()
    ...
    settlement_keywords = (
        config.DEFAULT_SETTLEMENT_KEYWORDS
        if hasattr(config, "DEFAULT_SETTLEMENT_KEYWORDS")
        else ["venmo", "zelle", "cash app", "paypal"]
    )
    ...
    if not simple_values:
        ...
        return (
            output_dir / "placeholder_no_data.html",
            "No flow data for Sankey diagram.",
        )
...
def build_data_quality_table_viz(
    ...
    if ledger_df.empty or "DataQualityFlag" not in ledger_df.columns:
        ...
        return (
            output_dir / "placeholder_no_data.html",
            "No data for data quality table visualization.",
        )
    ...
    combined = (
        pd.concat([flagged_rows, high_impact_rows])
        .drop_duplicates()
        .reset_index(drop=True)
    )
    if combined.empty:
        logger_instance.info(
            "No data quality issues or high impact transactions found for table visualization."
        )
        return (
            output_dir / "placeholder_no_data.html",
