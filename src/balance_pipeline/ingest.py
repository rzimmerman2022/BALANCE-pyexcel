...
def _find_schema(
    ...
    if not _SCHEMAS:
        raise FatalSchemaError(
            "Schema registry is empty or failed to load. Cannot match schema."
        )
    ...
    for schema in _SCHEMAS:
        ...
        try:
            ...
            if not filename_pattern:
                logging.warning(
                    f"Schema '{schema_id}' missing 'match_filename'. Skipping."
                )
            ...
            if not required_headers:
                ...
                logging.info(
                    f"Schema '{schema_id}' matched based on filename only for: {csv_path.name}"
                )
                return schema
            else:
            ...
            logging.info(
                f"Schema '{schema_id}' matched headers and filename for: {csv_path.name}"
            )
            return schema
            ...
            logging.error(
                f"Error while evaluating schema '{schema_id}' for {csv_path.name}: {e}"
            )
    ...
    try:
        if rule == "flip_if_withdrawal":
            ...
            if "Category" in df.columns:
                ...
                withdrawal_mask = df["Category"].notna() & df["Category"].str.contains(
                    "Withdrawal|Payment", case=False, na=False, regex=True
                )
                # For matched rows, ensure the Amount is negative (take absolute value, then negate).
                df.loc[withdrawal_mask, "Amount"] = -df.loc[
                    withdrawal_mask, "Amount"
                ].abs()
                logging.info(
                    "Applied 'flip_if_withdrawal' sign rule using 'Category' column."
                )
            else:
                # Log a warning if the required Category column is missing for this rule.
                logging.warning(
                    "Sign rule 'flip_if_withdrawal' requires a 'Category' column, which was not found."
                )
        ...
        # Add other potential rules here if needed later (e.g., flip_if_negative)
            ...
            logging.warning(
                f"Unknown sign_rule '{rule}' specified in schema. Amount signs left 'as_is'."
            )
...
def _derive_columns(
    ...
    for (
        new_col_name,
        rule_cfg,
    ) in derived_cfg.items():  # Renamed rule_config to rule_cfg for clarity
    ...
    **kwargs: Any,
    ...
    if not folder.is_dir():
        ...
        raise FileNotFoundError(
            f"CSV inbox path not found or not a directory: {folder}"
        )
    if not _SCHEMAS:
        # Cannot proceed without rules defined in the schema registry.
        raise ValueError(
            "Schema registry is empty or failed to load. Cannot process files."
        )
    ...
    logging.info(
        f"Found {len(all_csv_files)} CSV files initially in {folder} and its subdirectories."
    )
    ...
    for csv_path in all_csv_files:
        ...
        relative_path_str = str(csv_path.relative_to(folder)).replace(
            "\\", "/"
        ...
        for pattern in exclude_patterns:
            if csv_path.match(pattern) or Path(relative_path_str).match(pattern):
                logging.debug(
                    f"Excluding '{csv_path.name}' due to exclude pattern: '{pattern}'"
                )
                excluded = True
                break
        if excluded:
        ...
        if only_patterns:
            ...
            if not included:
                logging.debug(
                    f"Skipping '{csv_path.name}' as it does not match any 'only' patterns: {only_patterns}"
                )
                continue
    ...
    logging.info(
        f"Processing {len(files_to_process)} CSV files after applying exclude/only filters."
    )
    ...
    for csv_path in files_to_process:
        ...
        try:
            ...
            if schema is None:
                logging.warning(
                    f"Skipping file: No matching schema found for {csv_path.name}"
                )
                continue
            ...
            for col_name, col_value in extra_static_cols.items():
                ...
                logging.info(
                    f"Added static column '{col_name}' with value '{col_value}' to all rows."
                )
            ...
            df = _derive_columns(
                df, derived_cfg
            )  # Pass derived_cfg directly, new function handles None
            ...
            if extras_to_ignore:
                cols_to_drop_from_df = [
                    col for col in extras_to_ignore if col in df.columns
                ]
                if cols_to_drop_from_df:
                    df = df.drop(columns=cols_to_drop_from_df)
                    logging.info(
                        f"Dropped columns from DataFrame based on 'extras_ignore': {cols_to_drop_from_df} for schema '{schema_id}'"
                    )
            ...
            if missing_essentials:
                logging.error(
                    f"Essential columns {missing_essentials} missing after mapping/derivation for {csv_path.name} (Schema: {schema_id}). Skipping file."
                )
                continue

            # Normalize core data types (errors='coerce' turns failures into NaT/NaN).
            date_format = schema.get(
                "date_format"
            )  # Get format from schema if specified
            ...
            if date_format:
                ...
                logging.debug(
                    f"Using specified date format '{date_format}' for schema '{schema_id}'."
                )
                ...
                logging.debug(
                    f"Applying explicit '%m/%d' date parsing for schema '{schema_id}'. Year will be inferred by pandas (likely current year)."
                )
                ...
                logging.debug(
                    f"No date format specified for schema '{schema_id}'. Using pandas default inference (month-first)."
                )
                df["Date"] = pd.to_datetime(
                    df["Date"], errors="coerce", dayfirst=False
                )  # Explicitly month-first default
            ...
            if len(df) < initial_rows:
                logging.warning(
                    f"Dropped {initial_rows - len(df)} rows from {csv_path.name} due to invalid Date or Amount."
                )
            if df.empty:
                logging.warning(
                    f"File became empty after dropping invalid rows: {csv_path.name}. Skipping file."
                )
                continue
            ...
            output_df = (
                pd.DataFrame()
            )  # Start fresh to ensure correct order and columns
            for col in STANDARD_COLS:
                if col in df.columns:
                    ...
                    output_df[col] = pd.NA
                    logging.debug(
                        f"Standard column '{col}' not found for {csv_path.name}; added with NA."
                    )

            # Append the processed DataFrame to our list.
            parts.append(output_df)
            logging.info(
                f"Successfully processed and standardized {csv_path.name} ({len(output_df)} rows)."
            )
        ...
        # --- Handle File-Specific Errors ---
            ...
            raise RecoverableFileError(
                f"File disappeared during processing: {csv_path}"
            ) from e
        except Exception as e:
            raise RecoverableFileError(
                f"Failed to process file {csv_path.name}: {e}"
            ) from e
    ...
    if not parts:
        logging.warning(
            f"No CSV files could be successfully processed under {folder}. Returning empty DataFrame."
        )
    ...
    logging.info(
        f"Finished processing folder. Total rows ingested across {len(parts)} file(s): {len(combined_df)}"
    )
    return combined_df
