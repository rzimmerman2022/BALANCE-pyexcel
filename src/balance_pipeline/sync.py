...
# ------------------------------------------------------------------------------
def sync_review_decisions(df_transactions: pd.DataFrame, df_queue_review: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    log.info(f"Starting sync process. Transactions: {len(df_transactions)} rows, Queue: {len(df_queue_review)} rows.")
    ...
    if df_queue_review.empty:
        log.info("Queue review DataFrame is empty. No decisions to sync. Returning unchanged transactions.")
        return df_transactions.copy()
    ...
    if not all(col in df_transactions.columns for col in required_trans_cols):
        log.error(f"Transactions DataFrame missing required columns for sync. Need: {required_trans_cols}")
        # Optionally raise an error or return unchanged
        return df_transactions.copy()
    if not all(col in df_queue_review.columns for col in required_queue_cols):
        log.error(f"Queue Review DataFrame missing required columns for sync. Need: {required_queue_cols}")
    ...
    df_queue_filtered = df_queue_filtered[df_queue_filtered[QUEUE_DECISION_COL].notna() & 
    ...
    def standardize_decision(val):
        ...
        if val in ['Y', 'N', 'S']:
            ...
            log.warning(f"Invalid decision value: '{val}'. Expected Y, N, or S. Ignoring.")
            return None
    
    # Create new column with standardized values
    df_queue_filtered['SharedFlag_update'] = df_queue_filtered[QUEUE_DECISION_COL].apply(standardize_decision)
    
    # 3. Handle split percentages
    # Only relevant when SharedFlag is 'S', should be between 0 and 100
    def validate_split_percent(row):
        if row['SharedFlag_update'] != 'S':
            return None
        
        val = row[QUEUE_SPLIT_COL]
        try:
            if pd.isna(val):
                log.warning(f"Missing split percentage for split decision on TxnID: {row[QUEUE_TXNID_COL]}. Using 50%.")
                return 50.0
            
            val = float(val)
            # Clamp to range 0-100
            if val < 0:
                log.warning(f"Split percentage ({val}) below 0 for TxnID: {row[QUEUE_TXNID_COL]}. Clamping to 0.")
                return 0.0
            elif val > 100:
                log.warning(f"Split percentage ({val}) above 100 for TxnID: {row[QUEUE_TXNID_COL]}. Clamping to 100.")
                return 100.0
            return val
        except (ValueError, TypeError):
            log.warning(f"Invalid split percentage format for TxnID: {row[QUEUE_TXNID_COL]}. Using 50%.")
            return 50.0
    
    df_queue_filtered['SplitPercent_update'] = df_queue_filtered.apply(validate_split_percent, axis=1)
    
    # 4. Remove rows where we couldn't standardize the decision
    df_queue_filtered = df_queue_filtered[df_queue_filtered['SharedFlag_update'].notna()]
    
    # 5. Select only relevant columns for merging
    df_processed_decisions = df_queue_filtered[[QUEUE_TXNID_COL, 'SharedFlag_update', 'SplitPercent_update']]
    
    log.info(f"Processed {len(df_processed_decisions)} valid decisions from Queue_Review.")
    ...
    if mask_update_shared.any():
        merged_df.loc[mask_update_shared, TRANS_SHARED_FLAG_COL] = merged_df.loc[mask_update_shared, 'SharedFlag_update']
    ...
    if mask_update_split.any():
        merged_df.loc[mask_update_split, TRANS_SPLIT_PERC_COL] = merged_df.loc[mask_update_split, 'SplitPercent_update']
        log.info(f"Updated SplitPercent for {mask_update_split.sum()} transactions.")
    
    # Remove the temporary columns used for updating
    final_df = merged_df.drop(columns=['SharedFlag_update', 'SplitPercent_update'], errors='ignore')
