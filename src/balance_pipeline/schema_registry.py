...
def _find_matching_schema_main_impl(headers: Iterable[str]) -> MatchResult:  # Renamed
    ...
    for schema_rules_dict in _ALL_LOADED_SCHEMAS:
        ...
        if schema_canonical_signature.issubset(canonical_headers):
            ...
            score = len(schema_canonical_signature) # Higher score for more specific signatures
            
            missing_from_csv = schema_canonical_signature - canonical_headers # Should be empty if subset match
            ...
            current_match_result = MatchResult(
                ...
                schema=Schema(name=schema_id, required={}, optional={}), # Placeholder Schema object
                rules=schema_rules_dict, # The full rules dictionary
                score=(score, -len(extras_in_csv)), # Prioritize more matched, fewer extras
                missing=missing_from_csv, 
                extras=extras_in_csv
            )

            if best_match_result is None or current_match_result.score > best_match_result.score:
