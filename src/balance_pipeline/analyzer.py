...
#
# Description : CLI utility that merges expense, ledger, and rent CSV exports
#                into a single master ledger with analytics and reporting.
# Key Concepts:
#   - Auto-detection of header rows in raw CSV files
#   - Running balance reconciliation across multiple sources
#   - Data quality logging and visualization
# Public API  :
#   - main() -> None
#   - DataLoaderV23.load_expense_history(path: Path) -> pd.DataFrame
#   - DataLoaderV23.load_transaction_ledger(path: Path) -> pd.DataFrame
#   - merge_expense_and_ledger_data(expense_hist: pd.DataFrame,
#                                   transaction_ledger: pd.DataFrame)
# ---------------------------------------------------------------------------
# Change Log
# Date        Author       Type       Note
# 2024-06-02  Codex        feat       Auto-detect ledger header and logging
# 2025-05-30  Financial Analysis System  feat  Initial creation of the module.
###############################################################################
...
class DataLoaderV23:
    ...
    def load_transaction_ledger(path: Path) -> pd.DataFrame:
        ...
        if raw.empty:
            logger.error("Transaction Ledger file is empty.")
            return pd.DataFrame()

        header_row: int | None = None
        for idx in range(min(5, len(raw))):
            row = raw.iloc[idx].astype(str).str.lower()
            if (
                row.str.contains("date").any()
                or row.str.contains("running balance").any()
            ):
                header_row = idx
                break
...
class EnhancedSharedExpenseAnalyzer:
    ...
    def analyze(self) -> dict[str, Any]:
        ...
        try:
            ...
            master_ledger = self._create_master_ledger(
                rent_df, expense_df, transaction_ledger_raw
            )
    ...
    def _create_master_ledger(
        ...
        logger.info(
            "Creating master ledger v2.3 from processed rent, expense and ledger data..."
        )
        ...
        # Normalize ledger columns if provided
        if ledger_df is not None and not ledger_df.empty:
            ...
            ledger_df.rename(columns=rename_map, inplace=True)
            if "BalanceImpact" not in ledger_df.columns:
                if "RunningBalance" in ledger_df.columns:
                    ledger_df["BalanceImpact"] = (
                        ledger_df["RunningBalance"]
                        .diff()
                        .fillna(ledger_df["RunningBalance"])
                    )
                else:
                    ledger_df["BalanceImpact"] = 0.0
            ledger_df["TransactionType"] = "LEDGER"
            ledger_df["AllowedAmount"] = ledger_df.get(
                "AllowedAmount", ledger_df["ActualAmount"]
            )
            ledger_df["IsShared"] = False
            ledger_df["RyanOwes"] = 0.0
            ledger_df["JordynOwes"] = 0.0
            ledger_df["AuditNote"] = ledger_df.get("AuditNote", "")
            ledger_df["DataQualityFlag"] = DataQualityFlag.CLEAN.value
            if "Merchant" not in ledger_df.columns:
                ledger_df["Merchant"] = ""
        ...
        for df_iter, name in [
            ...
            if "RunningBalance" not in df_iter.columns:
                df_iter["RunningBalance"] = np.nan

        # Filter out completely empty dataframes before concat to avoid issues with all-NA columns
        dfs_to_concat = []
        if not rent_df.empty:
            dfs_to_concat.append(rent_df[common_cols + ["RunningBalance"]])
        if not expense_df.empty:
            dfs_to_concat.append(expense_df[common_cols + ["RunningBalance"]])
        if ledger_df is not None and not ledger_df.empty:
            dfs_to_concat.append(ledger_df[common_cols + ["RunningBalance"]])
...
def main():
    ...
    for file_path, file_name in files_to_check:
        if not file_path.exists():
            logger.error(
                "Error: %s file not found at %s", file_name, file_path.resolve()
            )
    ...
    logger.info(
        "Using Configuration: Ryan Pct=%.1f%%, Jordyn Pct=%.1f%%, Rent Baseline=$%.2f",
        config.RYAN_PCT * 100,
        config.JORDYN_PCT * 100,
        config.RENT_BASELINE,
    )
    ...
    try:
        ...
        logger.info(
            "Final Balance Reported: $%.2f",
            results["reconciliation"]["final_balance_reported"],
        )
        logger.info("Who Owes Whom: %s", results["reconciliation"]["who_owes_whom"])
        logger.info(
            "Amount Owed: $%.2f",
            results["reconciliation"]["amount_owed"],
        )
        logger.info(
