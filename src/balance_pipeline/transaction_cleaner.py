###############################################################################
# BALANCE-pyexcel – Comprehensive Transaction Cleaner
#
# Description : Data-driven patterns for standardizing descriptions and merchants.
# Key Concepts: - Regex-based text cleaning
#               - Merchant standardization lookup
#               - Recommendation loading from analysis files
# Public API  : - ComprehensiveTransactionCleaner
#               - ComprehensiveTransactionCleaner.clean_description()
#               - ComprehensiveTransactionCleaner.extract_original_merchant()
# -----------------------------------------------------------------------------
# Change Log
...
class ComprehensiveTransactionCleaner:
    ...
    def __init__(self, analysis_results_path: Optional[Path] = None):
        ...
        self.merchant_standardization: Dict[
            str, str
        ] = {}  # For merchant_variations.json
        self.analysis_results_path: Optional[Path] = (
            analysis_results_path  # Store for later use
        )

        if self.analysis_results_path:
            self.load_analysis_results(
                self.analysis_results_path
            )  # Loads full_column_analysis & recommendations
    ...
    def load_analysis_results(self, results_path: Path) -> None:
        ...
        try:
            ...
            if recommendations_file.exists():
                ...
                log.warning(
                    f"Cleaning recommendations file not found at {recommendations_file}"
                )
    ...
    def initialize_merchant_patterns(self) -> None:
        ...
        self.merchant_extraction_patterns = [
            ...
            (
                r"ZELLE\s+(PAYMENT\s+)?(FROM|TO)\s+([A-Z][A-Z\s]+?)(\s+[A-Z0-9]{8,})",
                r"ZELLE \2 \3",
            ),
            (
                r"VENMO\s+(PAYMENT\s+)?(FROM|TO)\s+([A-Z][A-Z\s]+?)(\s+[A-Z0-9]{8,})",
                r"VENMO \2 \3",
            ),
            # Direct deposits
            (
                r"([A-Z][A-Z\s]+?)\s+DIRECTPAY\s+[A-Z0-9]+\s+[A-Za-z\s]+$",
                r"\1 DIRECTPAY",
            ),
        ...
        if self.analysis_results_path:
            merchant_variations_file = (
                self.analysis_results_path / "merchant_variations.json"
            )
            if merchant_variations_file.exists():
                try:
                    ...
                    log.info(
                        f"Loaded {len(self.merchant_standardization)} merchant standardization rules from {merchant_variations_file}"
                    )
                except Exception as e:
                    log.error(
                        f"Error loading merchant variations from {merchant_variations_file}: {e}"
                    )
            else:
                log.warning(
                    f"Merchant variations file not found at {merchant_variations_file}. Standardization rules will be empty."
                )
        else:
            log.warning(
                "Analysis results path not provided. Merchant standardization rules will be empty."
            )
    ...
    def standardize_merchant(self, original_merchant: str) -> str:
        ...
        merchant = re.sub(
            r"\s+(LLC|INC|CORP|CO|LTD)\.?\s*$", "", merchant, flags=re.IGNORECASE
        )
    ...
    def clean_institution(self, institution: str) -> str:
        ...
        cleaned = re.sub(
            r"\s*(CHECKING|SAVINGS|CREDIT CARD).*$", "", cleaned, flags=re.IGNORECASE
        )
    ...
    def process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "OriginalDescription" in cleaned_df.columns:
            log.info(
                "Stage 1: Creating cleaned Description from OriginalDescription..."
            )
            cleaned_df["Description"] = cleaned_df["OriginalDescription"].apply(
            ...
            log.info(
                f"  Average length reduced from {orig_lengths:.1f} to {new_lengths:.1f} characters"
            )
        ...
        if "OriginalMerchant" not in cleaned_df.columns:
            cleaned_df["OriginalMerchant"] = pd.Series(
                [np.nan] * len(cleaned_df), index=cleaned_df.index, dtype=object
            )
        ...
        cleaned_df["OriginalMerchant"] = (
            cleaned_df["OriginalMerchant"].astype(str).replace("nan", "")
        )
        ...
        if "OriginalDescription" in cleaned_df.columns:
            missing_original_merchant_mask = pd.isna(cleaned_df["OriginalMerchant"]) | (
                cleaned_df["OriginalMerchant"] == ""
            )

            if missing_original_merchant_mask.any():
                log.info(
                    "  Extracting OriginalMerchant from OriginalDescription for missing values..."
                )
                cleaned_df.loc[missing_original_merchant_mask, "OriginalMerchant"] = (
                    cleaned_df.loc[
                        missing_original_merchant_mask, "OriginalDescription"
                    ].apply(self.extract_original_merchant)
                )
        else:
            log.warning(
                "  OriginalDescription column not found, cannot extract OriginalMerchant for missing values."
            )
        ...
        if "OriginalMerchant" in cleaned_df.columns:
            ...
            cleaned_df["Merchant"] = cleaned_df["OriginalMerchant"].apply(
                self.standardize_merchant
            )
            ...
            log.info(
                f"  Merchants consolidated from {orig_unique} to {new_unique} unique values"
            )
        ...
        if "Institution" in cleaned_df.columns:
            ...
            cleaned_df["Institution"] = cleaned_df["Institution"].apply(
                self.clean_institution
            )
        ...
        if (
            "Description" in cleaned_df.columns
            and "OriginalDescription" in cleaned_df.columns
        ):
            unchanged_desc = (
                cleaned_df["Description"] == cleaned_df["OriginalDescription"]
            ).sum()
            log.info(
                f"  Descriptions unchanged: {unchanged_desc} ({unchanged_desc/len(cleaned_df)*100:.1f}%)"
            )
...
def apply_comprehensive_cleaning(
    df: pd.DataFrame, analysis_path: Optional[Path] = None
) -> pd.DataFrame:
    """
