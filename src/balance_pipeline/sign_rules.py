...
def flip_if_positive(df: pd.DataFrame, amount_col: str) -> pd.DataFrame:
    ...
    if amount_col not in df.columns:
        raise DataConsistencyError(
            f"Column '{amount_col}' not found in DataFrame for flip_if_positive"
        )
    # Ensure numeric
...
def flip_if_withdrawal(
    df: pd.DataFrame, amount_col: str, category_col: str = "Category"
) -> pd.DataFrame:
    """
    ...
    if amount_col not in df.columns:
        raise DataConsistencyError(
            f"Column '{amount_col}' not found in DataFrame for flip_if_withdrawal"
        )
    if category_col not in df.columns:
        raise DataConsistencyError(
            f"Column '{category_col}' not found in DataFrame for flip_if_withdrawal"
        )
    # Ensure numeric
    df[amount_col] = pd.to_numeric(df[amount_col], errors="coerce")
    # Identify withdrawal categories
    mask = df[category_col].notna() & df[category_col].str.contains(
