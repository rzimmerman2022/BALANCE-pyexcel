...
def _categorize_merchant(
    merchant: str | None, categories_map: dict[str, list[str]] | None = None
) -> str:
    ...
    current_categories_map = (
        categories_map if categories_map is not None else DEFAULT_MERCHANT_CATEGORIES
    )
...
def _analyze_rent_budget_variance(
    ...
    if (
        rent_df.empty
        or "Budget_Variance" not in rent_df.columns
        or rent_df["Budget_Variance"].isna().all()
    ):
        logger_instance.warning(
            "No budget data available or all variances are NaN for rent budget analysis."
        )
    ...
    if not variance_data.empty:
        ...
        if pd.api.types.is_datetime64_any_dtype(variance_data[month_col]) or isinstance(
            variance_data[month_col].dtype, pd.PeriodDtype
        ):
            variance_data[month_col] = variance_data[month_col].astype(str)

        analysis = {
            "total_budget_variance": round(
                variance_data["Budget_Variance"].sum(), config.CURRENCY_PRECISION
            ),
            "avg_monthly_variance": round(
                variance_data["Budget_Variance"].mean(), config.CURRENCY_PRECISION
            ),
        ...
        if not variance_data[variance_data["Budget_Variance"] > 0.005].empty:
            ...
            analysis["largest_overrun"] = {
                ...
                "amount": round(
                    variance_data.loc[overrun_idx, "Budget_Variance"],
        ...
        if not variance_data[variance_data["Budget_Variance"] < -0.005].empty:
            ...
            analysis["largest_underrun"] = {
                ...
                "amount": round(
                    variance_data.loc[underrun_idx, "Budget_Variance"],
                    config.CURRENCY_PRECISION,
                ),
            }
...
def perform_advanced_analytics(
    ...
    shared_ledger = valid_dates_ledger[
    ...
    if (
        ...
        high_balance_transactions = valid_dates_ledger[
            ...
            > config.LIQUIDITY_STRAIN_THRESHOLD
        ]
            ...
            liquidity_issues.append(
                {
                    ...
                    "running_balance": round(
                        row["RunningBalance"], config.CURRENCY_PRECISION
                    ),
    ...
    if not expense_only_df.empty and "Merchant" in expense_only_df.columns:
        expense_only_df["Category"] = expense_only_df["Merchant"].apply(
            lambda m: _categorize_merchant(
                m,
                config.DEFAULT_MERCHANT_CATEGORIES
                if hasattr(config, "DEFAULT_MERCHANT_CATEGORIES")
                else None,
            )
        )
        category_summary = (
        ...
        if not category_summary.empty:
            ...
            analytics["expense_category_analysis"] = {
                ...
                "pareto_80_amount_sum": round(
                    category_totals.loc[pareto_80_categories].sum(),
                    config.CURRENCY_PRECISION,
                )
    ...
    if not shared_ledger.empty and "AllowedAmount" in shared_ledger.columns:
        ...
        if len(monthly_shared_spending) >= 3:
            ...
            analytics["monthly_shared_spending_trend"] = {
                ...
                "monthly_values": {
                    d.strftime("%Y-%m"): round(val, config.CURRENCY_PRECISION)
                    for d, val in monthly_shared_spending.items()
                },
            }
            if (
                not monthly_shared_spending.index.empty
                and pd.notna(slope)
                and pd.notna(intercept)
            ):  # Ensure slope/intercept are valid
                future_x = np.arange(
                ...
                analytics["monthly_shared_spending_forecast"] = dict(
                    zip(
                        ...
                        np.round(forecast_values, config.CURRENCY_PRECISION),
                        strict=False,
                    )
                )
            else:
                analytics["monthly_shared_spending_forecast"] = {
                    "message": "Not enough data or invalid trend for forecast."
                }
        else:
            analytics["monthly_shared_spending_trend"] = {
        ...
        analytics["monthly_shared_spending_trend"] = {
            "message": "No shared spending data for trend."
        }
        analytics["monthly_shared_spending_forecast"] = {
    ...
    if not processed_rent_df.empty:
        analytics["rent_budget_analysis"] = _analyze_rent_budget_variance(
            processed_rent_df, config, logger_instance
        )
    else:
        analytics["rent_budget_analysis"] = {
            "message": "Processed rent data is empty, skipping budget variance analysis."
        }
...
def _calculate_data_quality_score(
    master_ledger: pd.DataFrame, logger_instance: logging.Logger = logger
) -> float:
...
def comprehensive_risk_assessment(
    ...
    validation_summary: dict[
        str, Any
    ],  # Pass validation_summary from main orchestrator
    config: AnalysisConfig,
    logger_instance: logging.Logger = logger,
) -> dict[str, Any]:
    ...
    dq_score = _calculate_data_quality_score(
        master_ledger, logger_instance
    )  # Use helper
    ...
    if (
        "potential_liquidity_strain_points" in analytics_results
        and analytics_results["potential_liquidity_strain_points"]
    ):
    ...
    if (
        ...
        and abs(rent_budget_analysis.get("avg_monthly_variance", 0))
        > config.RENT_BASELINE * 0.05
    ):  # e.g. 5% of baseline
        risks["details"].append(
            {
    ...
    if validation_summary.get("ledger_balance_match") not in [
        "Matched",
