...
"""
    ...
    for viz_name, viz_path_str in sorted(visualizations.items()):
        viz_file_path = Path(
            viz_path_str
        )  # Use the full path for checking suffix, but relative for src
    ...
    try:
        ...
        logger_instance.error(
            f"Permission denied writing HTML dashboard {dashboard_path}: {e}"
        )
        raise
    except OSError as e:
        ...
        logger_instance.error(
            f"Encoding error writing HTML dashboard {dashboard_path}: {e}"
        )
        raise
...
def _generate_executive_summary_pdf(
    ...
    styles.add(
        ParagraphStyle(
            name="ItalicCustom", fontName="Helvetica-Oblique", fontSize=9, leading=11
        )
    )
    ...
    story.append(
        Paragraph(
            f"Report Date: {datetime.now(UTC).strftime('%B %d, %Y %Z')}",
            styles["Normal"],
        )
    )
    ...
    for viz_name, viz_path_str in sorted(visualizations.items()):
        ...
        if viz_file_path.suffix == ".png":
            ...
            try:
                ...
                target_frame_height = (
                    A4[0] - 1 * inch - 1 * inch
                )  # landscape A4 height - margins - space for title/alt

                scale_w = (
                    target_frame_width / img_width_pts
                    if img_width_pts > target_frame_width
                    else 1.0
                )
                scale_h = (
                    target_frame_height / img_height_pts
                    if img_height_pts > target_frame_height
                    else 1.0
                )
                ...
                logger_instance.warning(
                    f"Placeholder image missing; skipping image block: {viz_file_path.name} - {img_err}"
                )
                story.append(
                    Paragraph(
                        f"[Image not available: {viz_file_path.name}]",
                        styles["ItalicCustom"],
                    )
                )
            ...
            if alt:
                story.append(Paragraph(f"{alt}", styles["ItalicCustom"]))
            story.append(Spacer(1, 0.2 * inch))
    ...
    try:
        ...
        try:
            ...
            logger_instance.error(
                f"Failed to write fallback text report: {fallback_err}"
            )
...
def generate_all_outputs(
    ...
    master_ledger_export["Category_Display"] = master_ledger_export.get(
        "TransactionType", pd.NA
    )
    master_ledger_export["Amount_Charged_Display"] = master_ledger_export.get(
        "ActualAmount", pd.NA
    )
    master_ledger_export["Shared_Amount_Display"] = master_ledger_export.get(
        "AllowedAmount", pd.NA
    )
    master_ledger_export["Cumulative_Balance_Display"] = master_ledger_export.get(
        "RunningBalance", pd.NA
    )
    ...
    if not master_ledger_export.empty:
        ...
        try:
            ...
            logger_instance.error(
                f"Failed to save line_by_line_reconciliation_v2.3.csv: {e}"
            )
    ...
    dq_score = analytics_results.get(
        "data_quality_score", 0.0
    )  # Get it from analytics if available
    ...
    summary_data = {
        ...
        if reconciliation_results.get("reconciled", False)
        else "NO",
        "Ledger Balance Matched": str(
            analytics_results.get("validation_summary", {}).get(
                "ledger_balance_match", "N/A"
            )
        ),  # Assuming validation summary is in analytics
    ...
    if data_quality_issues:
        ...
        try:
            ...
            logger_instance.error(
                f"Failed to save data_quality_issues_log_v2.3.csv: {e}"
            )
    else:
    ...
    try:
        with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
            ...
            if not master_ledger_export.empty:
                ...
                master_ledger_export[excel_ledger_cols].to_excel(
                    writer, sheet_name="Ledger Highlights", index=False
                )

            pd.DataFrame(
                list(reconciliation_results.items()), columns=["Metric", "Value"]
            ).to_excel(writer, sheet_name="Reconciliation Details", index=False)
            if (
                "category_details" in reconciliation_results
                and reconciliation_results["category_details"]
            ):
                pd.DataFrame(reconciliation_results["category_details"]).to_excel(
                    writer, sheet_name="Recon Category Breakdown", index=False
                )

            if "expense_category_analysis" in analytics_results and isinstance(
                analytics_results["expense_category_analysis"].get("summary_table"),
                list,
            ):
                pd.DataFrame(
                    analytics_results["expense_category_analysis"]["summary_table"]
        ...
        logger_instance.error(
            f"Failed to generate Excel report {excel_path}: {e_excel}", exc_info=True
        )

    # 7. Dashboard HTML
    try:
        dashboard_path = _generate_dashboard_html(
            visualizations, alt_texts, output_dir, logger_instance
        )
        output_paths["dashboard_html"] = str(dashboard_path)
        # logger_instance.info(f"Saved: {dashboard_path}") # Already logged in helper
    except Exception as e_dash:
        logger_instance.error(
            f"Failed to generate dashboard HTML: {e_dash}", exc_info=True
        )

    # 8. Executive PDF
    try:
        pdf_path = _generate_executive_summary_pdf(
            summary_data,
            visualizations,
            alt_texts,
            recommendations,
        ...
        logger_instance.error(
            f"Failed to generate executive PDF: {e_pdf}", exc_info=True
        )
