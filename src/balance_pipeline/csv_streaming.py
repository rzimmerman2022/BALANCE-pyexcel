...
def read_csv_chunked(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        logger.info(
            f"Completed reading {filepath.name}: {chunk_count} chunks processed"
        )
...
def process_csv_file_streaming(
    ...
    **csv_kwargs: Any,
    ...
    logger.info(
        f"Combined {len(processed_chunks)} chunks into {len(result)} total rows"
    )
...
def estimate_memory_usage(
    filepath: str | Path, sample_size: int = 1000
) -> dict[str, float]:
    """
...
def should_use_streaming(
    filepath: str | Path, memory_threshold_mb: float = 500.0
) -> bool:
    """
    ...
    try:
        ...
        logger.warning(
            f"Could not estimate memory usage: {e}. Using file size heuristic."
        )
