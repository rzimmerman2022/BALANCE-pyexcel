    ...
    try:
        ...
        if schema_path is not None:
            ...
            if not schema_registry_override.exists():
                raise FileNotFoundError(
                    f"Schema registry not found: {schema_registry_override}"
                )
        ...
        if merchant_path is not None:
            ...
            if not merchant_lookup_override.exists():
                raise FileNotFoundError(
                    f"Merchant lookup not found: {merchant_lookup_override}"
                )
...
def save_output(df: pd.DataFrame, output_path: str | None, output_format: str) -> None:
    """
...
Examples:
  ...
  python -m balance_pipeline.main process *.csv --output processed.parquet --format parquet
    ...
    process_parser = subparsers.add_parser(
        "process", help="Process CSV files through the pipeline"
    )

    process_parser.add_argument("files", nargs="+", help="CSV files to process")

    process_parser.add_argument(
        "-s", "--schema-path", type=str, help="Path to custom schema registry YAML file"
    )
