...
""" 
    ...
    try:
        ...
        if not args.dry_run and parquet_path.exists(): # Don't load if dry run, might not be needed
            log.info(f"Loading existing canonical data from {parquet_path}...")
            try:
                canonical_df = pd.read_parquet(parquet_path)
                # Ensure key columns for merge exist, fill with defaults if not
                # This handles older parquet files that might not have these columns
                if "TxnID" not in canonical_df.columns:
                    log.warning(f"Canonical Parquet {parquet_path.name} missing 'TxnID'. Sync of previous states might be incomplete.")
                    # Add TxnID as None to allow merge to proceed, though it won't match anything.
                    # Or, consider this an error state depending on requirements.
                    # For now, let it proceed; new TxnIDs will be generated by etl_main.
                if "SharedFlag" not in canonical_df.columns:
                    canonical_df["SharedFlag"] = "?"
                if "SplitPercent" not in canonical_df.columns:
                    canonical_df["SplitPercent"] = pd.NA
                ...
                log.info(f"Loaded {len(canonical_df)} rows from canonical Parquet.")
            ...
            log.info(f"Canonical Parquet file not found at {parquet_path}. Will create if not dry run.")
        ...
        if not canonical_df.empty and "TxnID" in canonical_df.columns and "TxnID" in df_from_sources.columns:
            ...
            if "SharedFlag" in canonical_df.columns: class_cols_from_canonical.append("SharedFlag")
            if "SplitPercent" in canonical_df.columns: class_cols_from_canonical.append("SplitPercent")
