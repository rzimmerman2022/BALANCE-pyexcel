...
def run_analysis_pipeline(
    ...
    if config.debug_mode:
        ...
        if not expense_hist_raw.empty:
            expense_hist_raw.to_csv(
                output_dir / "01a_expense_history_raw.csv", index=False
            )
        if not transaction_ledger_raw.empty:
            transaction_ledger_raw.to_csv(
                output_dir / "01b_transaction_ledger_raw.csv", index=False
            )
        if not rent_alloc_raw.empty:
            rent_alloc_raw.to_csv(
                output_dir / "01c_rent_allocation_raw.csv", index=False
            )
        if not rent_hist_raw.empty:
            rent_hist_raw.to_csv(output_dir / "01d_rent_history_raw.csv", index=False)
    ...
    if expense_hist_raw.empty and transaction_ledger_raw.empty:
        logger.error(
            "Critical: Both Expense History and Transaction Ledger are empty. Aborting."
        )
    ...
    merged_expenses_ledger = merge_expense_and_ledger_data(
        expense_hist_raw, transaction_ledger_raw
    )
    merged_rent_data_full = merge_rent_data(
        rent_alloc_raw, rent_hist_raw
    )  # Contains budget info

    if config.debug_mode:
        if not merged_expenses_ledger.empty:
            merged_expenses_ledger.to_csv(
                output_dir / "02a_merged_expenses_ledger.csv", index=False
            )
        if not merged_rent_data_full.empty:
            merged_rent_data_full.to_csv(
                output_dir / "02b_merged_rent_data_full.csv", index=False
            )
    ...
    try:
        ...
        if rules_path.exists():
            ...
            logger.info(
                f"Loaded external rules from {rules_path}: {list(rules.keys())}"
            )
        else:
    ...
    processed_expenses = expense_pipeline(
        merged_expenses_ledger,
        config,
        rules,
        data_quality_issues_list,
        logger_instance=logger,
    )
    processed_rent = rent_pipeline(
        merged_rent_data_full,
        config,
        rules,
        data_quality_issues_list,
        logger_instance=logger,
    ...
    if config.debug_mode:
        if not processed_expenses.empty:
            processed_expenses.to_csv(
                output_dir / "03a_processed_expenses.csv", index=False
            )
        if not processed_rent.empty:
            processed_rent.to_csv(output_dir / "03b_processed_rent.csv", index=False)
    ...
    master_ledger_df = create_master_ledger(
        processed_rent, processed_expenses, config, logger_instance=logger
    )
    ...
    reconciliation_results = triple_reconciliation(
        master_ledger_df, config, logger_instance=logger
    )

    # Perform validation against ledger if data available
    if (
        not transaction_ledger_raw.empty
        and "Running Balance" in transaction_ledger_raw.columns
    ):
        ledger_final_balance = (
            transaction_ledger_raw["Running Balance"].iloc[-1]
            if not transaction_ledger_raw.empty
            else 0.0
        )
        master_final_balance = (
            master_ledger_df["RunningBalance"].iloc[-1]
            if not master_ledger_df.empty
            else 0.0
        )
        diff = abs(ledger_final_balance - master_final_balance)
        validation_summary["ledger_balance_match"] = (
            "Matched" if diff <= 0.015 else f"Mismatch (Diff: ${diff:,.2f})"
        )
    else:
        validation_summary["ledger_balance_match"] = "Skipped - No Ledger Data"
    validation_summary["reconciliation_match_strict"] = reconciliation_results.get(
        "reconciled", False
    )

    # Pass processed_rent_df to analytics for rent budget analysis
    analytics_results = perform_advanced_analytics(
        master_ledger_df, processed_rent, config, logger_instance=logger
    )
    analytics_results["data_sources_summary"] = (
        data_sources_summary  # Add this for reporting
    )
    analytics_results["validation_summary"] = (
        validation_summary  # Add validation summary for reporting
    )

    risk_assessment_results = comprehensive_risk_assessment(
        master_ledger_df,
        analytics_results,
        validation_summary,
        config,
    ...
    if not master_ledger_df.empty:
        try:
            path, alt = build_running_balance_timeline(
                master_ledger_df.copy(), config, Path("analysis_output"), logger
            )
            visualizations_paths["running-balance-timeline"] = str(path)
            alt_texts["running-balance-timeline"] = alt
        except Exception as e:
            logger.error(f"Failed running balance timeline viz: {e}", exc_info=True)

        try:
            path, alt = build_waterfall_category_impact(
                master_ledger_df.copy(),
                config,
                Path("analysis_output"),
                plotly_theme,
                logger,
            )
            visualizations_paths["waterfall-category-impact"] = str(path)
            alt_texts["waterfall-category-impact"] = alt
        except Exception as e:
            logger.error(f"Failed waterfall viz: {e}", exc_info=True)
        ...
        if "monthly_shared_spending_trend" in analytics_results:
            try:
                path, alt = build_monthly_shared_trend(
                    analytics_results, Path("analysis_output"), logger
                )
                visualizations_paths["monthly-shared-trend"] = str(path)
                alt_texts["monthly-shared-trend"] = alt
            except Exception as e:
                logger.error(f"Failed monthly trend viz: {e}", exc_info=True)
    ...
    recommendations_list = [
        f"Final Balance: {reconciliation_results.get('who_owes_whom','N/A')} ${reconciliation_results.get('amount_owed',0):.2f}"
    ]
    ...
    logger.info(
        f"  Who Owes Whom: {reconciliation_results.get('who_owes_whom', 'N/A')}"
    )
    ...
    logger.info(
        f"  Overall Risk Level: {risk_assessment_results.get('overall_risk_level', 'N/A')}"
    )
    logger.info("=" * 80)
...
def main_cli():
    parser = argparse.ArgumentParser(
        description="BALANCE-pyexcel Shared Expense Analyzer (Pipeline v0.1)"
    )
    parser.add_argument(
        "--expense",
        type=Path,
        default=Path("data/Expense_History_20250527.csv"),
        help="Path to expense history CSV.",
    )
    parser.add_argument(
        "--ledger",
        type=Path,
        default=Path("data/Transaction_Ledger_20250527.csv"),
        help="Path to transaction ledger CSV.",
    )
    parser.add_argument(
        "--rent",
        type=Path,
        default=Path("data/Rent_Allocation_20250527.csv"),
        help="Path to rent allocation CSV.",
    )
    parser.add_argument(
        "--rent-hist",
        type=Path,
        default=Path("data/Rent_History_20250527.csv"),
    ...
    parser.add_argument(
        "--debug_mode", action="store_true", help="Enable debug mode for snapshot CSVs."
    )
    ...
    if not np.isclose(config.RYAN_PCT + config.JORDYN_PCT, 1.0):
        logger.critical(
            "Configuration Error: Ryan's and Jordyn's percentages do not sum to 1.0. Aborting."
        )
        sys.exit(1)
    ...
    try:
        ...
        logger.critical(
            f"CLI Aborted - Unexpected error in pipeline: {e}", exc_info=True
        )
        sys.exit(1)
