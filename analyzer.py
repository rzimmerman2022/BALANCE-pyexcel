...
# Design constants
TABLEAU_COLORBLIND_10 = ['#007ACC','#FFB000','#FE6100','#785EF0','#64B200','#FF4B97','#00C1FF','#648FFF','#DC267F','#FFB000']
...
class DataQualityFlag(Enum):
    ...
    RENT_BUDGET_VARIANCE_HIGH = "RENT_BUDGET_VARIANCE_HIGH" # Used for Rent Hist vs Alloc
...
class DataLoaderV23:
    ...
    def _money_to_float(series: pd.Series) -> pd.Series:
        ...
        cleaned = (str_series
                   ...
                   .replace(['', 'nan', 'None', '-', 'inf', '-inf'], '0'))  # Handle empty/null/inf
    ...
    def load_expense_history(path: Path) -> pd.DataFrame:
        ...
        try:
            raw = pd.read_csv(path, header=None, dtype=str) # Read all as string initially
        ...
        if header_idx is None:
            logger.warning("Could not find header row with 'Name' column in Expense History. Falling back to first row as headers.")
            header_idx = 0
        ...
        logger.info(f"Loaded {len(df)} expense records from Expense History after cleaning")
        return df
    ...
    def load_transaction_ledger(path: Path) -> pd.DataFrame:
        ...
        if len(raw) < 2:
            logger.error("Transaction Ledger file is too short to contain headers and data.")
            return pd.DataFrame()

        headers = raw.iloc[1].astype(str).str.strip()
        ...
        df = DataLoaderV23._ensure_unique_df_columns(df)
        logger.info(f"Loaded {len(df)} ledger records from Transaction Ledger with running balance")
        return df
    ...
    def load_rent_allocation(path: Path) -> pd.DataFrame:
        ...
        money_cols = [
            'Tax Base Rent - Residential - 2 bed 2 bath', 'Tax Garage - 1 Car Attached',
            'Tax Common Area - Common Area Maintenance', 'Tax Trash - Trash',
            'Tax Management - Management Fee', 'Tax Rent Control - Rent Control Fee',
            'Conservice', 'Gross Total', "Ryan's Rent (43%)", "Jordyn's Rent (57%)",
            'Previous Balance', 'Rent Difference', 'Other Adjustments', 'New Balance'
        ]
    ...
    def load_rent_history(path: Path) -> pd.DataFrame:
        ...
        if df.empty:
            ...
            return pd.DataFrame(columns=['LineItem', 'Month', 'Measure', 'Month_Date', 'Amount'])
        ...
        tidy['Month_Date'] = pd.to_datetime(tidy['Month'], format='%B %Y', errors='coerce')
        tidy['Amount'] = DataLoaderV23._money_to_float(tidy['Amount'])
        tidy = tidy.drop(columns=['Month_Measure'])
        tidy = tidy[(tidy['Amount'] != 0) & tidy['Amount'].notna()] # Keep only non-zero, non-NA amounts
    ...
    @staticmethod
    def validate_loaded_data(expense_hist: pd.DataFrame, 
                               transaction_ledger: pd.DataFrame,
                               rent_alloc: pd.DataFrame,
                               rent_hist: pd.DataFrame) -> Dict[str, Any]:
        validation_results = {
            'expense_history': {
                'rows': len(expense_hist),
                'date_range': (expense_hist['Date of Purchase'].min(), 
                               expense_hist['Date of Purchase'].max()) if not expense_hist.empty and 'Date of Purchase' in expense_hist and expense_hist['Date of Purchase'].notna().any() else (None, None),
                'total_amount': expense_hist['Actual Amount'].sum() if 'Actual Amount' in expense_hist.columns else 0
            },
            'transaction_ledger': {
                'rows': len(transaction_ledger),
                'has_running_balance': 'Running Balance' in transaction_ledger.columns,
                'final_balance': transaction_ledger['Running Balance'].iloc[-1] if 'Running Balance' in transaction_ledger.columns and not transaction_ledger.empty else None
            },
            'rent_allocation': {
                'rows': len(rent_alloc),
                'months_covered': len(rent_alloc['Month'].unique()) if 'Month' in rent_alloc.columns and rent_alloc['Month'].notna().any() else 0,
                'avg_gross_rent': rent_alloc['Gross Total'].mean() if 'Gross Total' in rent_alloc.columns and rent_alloc['Gross Total'].notna().any() else 0
            },
            'rent_history': {
                'rows': len(rent_hist),
                'unique_line_items': len(rent_hist['LineItem'].unique()) if not rent_hist.empty and 'LineItem' in rent_hist else 0,
                'has_budget_actual': sorted(rent_hist['Measure'].unique().tolist()) if not rent_hist.empty and 'Measure' in rent_hist else []
            }
        }
...
def merge_expense_and_ledger_data(expense_hist: pd.DataFrame, 
                                  transaction_ledger: pd.DataFrame) -> pd.DataFrame:
    ...
    for df_ in [expense_hist, transaction_ledger]:
        if 'Date of Purchase' not in df_.columns: df_['Date of Purchase'] = pd.NaT
        if 'Actual Amount' not in df_.columns: df_['Actual Amount'] = 0.0
        if 'Name' not in df_.columns: df_['Name'] = "Unknown"
        if 'Merchant' not in df_.columns: df_['Merchant'] = "Unknown"
    ...
    for df_ in [expense_hist_c, transaction_ledger_c]:
        for kf in key_fields:
            if kf not in df_.columns:
                logger.warning(f"Key field '{kf}' missing in {df_['Source'].iloc[0] if not df_.empty else 'a dataframe'}. Deduplication might be affected.")
                # Add placeholder if critical for concat/drop_duplicates, though ideally columns should align
                if kf == 'Name' or kf == 'Merchant': df_[kf] = "MissingData"
                elif kf == 'Date of Purchase': df_[kf] = pd.NaT
                elif kf == 'Actual Amount': df_[kf] = 0.0
    ...
    dupes = [col for col in transaction_ledger_c.columns if col in expense_hist_c.columns]
    logger.debug(f"Columns present in both frames before concat: {dupes}")
    logger.debug(f"  Ledger dupes before concat -> {transaction_ledger_c.columns[transaction_ledger_c.columns.duplicated()].tolist()}")
    logger.debug(f"  Expense dupes before concat -> {expense_hist_c.columns[expense_hist_c.columns.duplicated()].tolist()}")

    combined = pd.concat([transaction_ledger_c, expense_hist_c], ignore_index=True, sort=False) # Ledger first
    ...
    combined = combined.sort_values(by=['Date of Purchase', 'Actual Amount', 'Source'], ascending=[True, True, True]) # True for Source means ExpenseHistory comes first if not preferring Ledger
    combined = combined.drop_duplicates(subset=key_fields, keep='first') # 'first' will keep TransactionLedger due to prior concat order

    logger.info(f"Merged expense and ledger data: {len(combined)} records from "
...
def merge_rent_data(rent_alloc: pd.DataFrame, rent_hist: pd.DataFrame) -> pd.DataFrame:
    ...
    if rent_hist.empty or 'Month_Date' not in rent_hist.columns:
        ...
        if 'Total_Actual' not in rent_alloc.columns: rent_alloc['Total_Actual'] = np.nan
        if 'Total_Budgeted' not in rent_alloc.columns: rent_alloc['Total_Budgeted'] = np.nan
        if 'Budget_Variance' not in rent_alloc.columns: rent_alloc['Budget_Variance'] = np.nan
        if 'Budget_Variance_Pct' not in rent_alloc.columns: rent_alloc['Budget_Variance_Pct'] = np.nan
        return rent_alloc.copy()

    rent_alloc_c = rent_alloc.copy()
    if 'Month_Date' not in rent_alloc_c.columns:
        logger.error("Rent Allocation is missing 'Month_Date' column. Cannot merge with Rent History.")
        return rent_alloc_c
    ...
    actual_by_month = (rent_actual.groupby('Month_Date')['Amount']
                       .sum().reset_index().rename(columns={'Amount': 'Total_Actual'}))
    budget_by_month = (rent_budget.groupby('Month_Date')['Amount']
                       .sum().reset_index().rename(columns={'Amount': 'Total_Budgeted'}))
    ...
    enhanced_rent['Budget_Variance'] = enhanced_rent['Total_Actual'].fillna(0) - enhanced_rent['Total_Budgeted'].fillna(0)
    
    # Avoid division by zero for percentage
    enhanced_rent['Budget_Variance_Pct'] = np.where(
        enhanced_rent['Total_Budgeted'].fillna(0) != 0,
        (enhanced_rent['Budget_Variance'] / enhanced_rent['Total_Budgeted'].replace(0, np.nan)) * 100,
...
class EnhancedSharedExpenseAnalyzer:
    ...
    def __init__(self, expense_file: Path, ledger_file: Path, 
                 rent_alloc_file: Path, rent_hist_file: Path,
        ...
        self.audit_trail: List[Dict[str, Any]] = [] # Consider if this is still used or replaced by logging
        self.validation_results: Dict[str, Any] = {} # Allow Any for more detailed validation
        ...
        for file_path, file_name in [(self.expense_file, "Expense History"),
    ...
    def _log_data_quality_issue(self, source: str, row_idx: Any, row_data: Dict[str, Any], flags: List[Union[DataQualityFlag, str]]):
        ...
        issue = {
            ...
            'row_data_sample': {k: sanitized_row_data.get(k) for k in ['Date', 'Payer', 'ActualAmount', 'AllowedAmount', 'Description'] if k in sanitized_row_data}
        }
    ...
    def _impute_missing_date(self, df: pd.DataFrame, idx: Any, date_col_name: str) -> pd.Timestamp:
        ...
        if not isinstance(idx, int):
            try:
                ...
                logger.error(f"Cannot find index {idx} in DataFrame for date imputation.")
                return pd.Timestamp.now(tz=timezone.utc).replace(day=1) + pd.offsets.MonthEnd(0) # Fallback
        else:
            idx_pos = idx
        ...
        if start_idx >= end_idx:
             logger.warning(f"Invalid slice for date imputation for index {idx} (pos {idx_pos}). Falling back.")
        ...
        if not nearby_dates.empty:
            ...
            if not nearby_dates_ts.empty:
                ...
                logger.warning(f"Could not convert nearby dates to Timestamps for row {idx}. Falling back.")
        else:
    ...
    def _process_expense_data(self, expense_hist: pd.DataFrame, 
                              transaction_ledger: pd.DataFrame) -> pd.DataFrame:
        logger.info("Processing and merging expense data from Expense History and Transaction Ledger...")
        ...
        if df.empty:
            ...
            essential_cols = ['Date', 'ActualAmount', 'AllowedAmount', 'Payer', 'Description', 'Merchant', 
                              'TransactionType', 'DataQualityFlag', 'IsShared', 'RyanOwes', 'JordynOwes', 'BalanceImpact', 'AuditNote']
        ...
        if 'Payer' not in df.columns: df['Payer'] = "Unknown"
        ...
        is_settlement_merchant = df['Merchant'].str.lower().str.strip().isin(settlement_merchants)
        ...
        for idx, row in df.iterrows(): # use iterrows carefully on large DFs
            ...
            if pd.isna(row['Date']):
                ...
                df.loc[idx, 'Date'] = self._impute_missing_date(df, idx, 'Date') # df, idx, col_name
            ...
            if quality_flags_for_row: # Only log if there are actual new flags for this row
        ...
        df.loc[is_settlement, 'AllowedAmount'] = 0 # Settlements are not shared expenses
        ...
        ryan_paid_jordyn = is_settlement & (df['Payer'].str.lower() == 'ryan') & \
                           (df['Description'].str.contains(r'to\s+jordyn', case=False, regex=True, na=False) | \
                            df['Merchant'].str.lower().isin(['venmo to jordyn', 'zelle to jordyn'])) # Example refined logic
        ...
        df.loc[non_settlement_mask, "PersonalPortion"] = (df.loc[non_settlement_mask, "ActualAmount"] - 
                                                          df.loc[non_settlement_mask, "AllowedAmount"]).round(self.config.CURRENCY_PRECISION)
        df.loc[non_settlement_mask, "IsShared"] = df.loc[non_settlement_mask, "AllowedAmount"] > 0.005 # Threshold for sharing
        ...
        df.loc[paid_by_ryan_shared_mask, "JordynOwes"] = (df.loc[paid_by_ryan_shared_mask, "AllowedAmount"] * self.config.JORDYN_PCT).round(self.config.CURRENCY_PRECISION)
        df.loc[paid_by_jordyn_shared_mask, "RyanOwes"] = (df.loc[paid_by_jordyn_shared_mask, "AllowedAmount"] * self.config.RYAN_PCT).round(self.config.CURRENCY_PRECISION)
        ...
        logger.info(f"Processed {len(df)} combined expense records. Detected {is_settlement.sum()} settlements.")
        return df

    def _handle_calculation_notes_in_processed_data(self, df: pd.DataFrame) -> pd.DataFrame:
        logger.info("Handling '2x to calculate' notes in processed expense descriptions...")
        ...
        for idx in df[two_x_mask].index:
            ...
            log_msg = (f"Row {idx}: '2x to calculate' found. "
        ...
        if modified_indices:
            logger.info(f"Applied '2x' calculation to 'AllowedAmount' for {len(modified_indices)} rows based on description note.")
        return df
    ...
    def _detect_duplicates_in_processed_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if num_duplicates > 0:
            logger.warning(f"Detected {num_duplicates} potential duplicate transactions in merged data.")
            for idx in df[duplicates_mask].index: # df.index should be unique after reset_index in merge
                self._update_row_data_quality_flags(df, idx, [DataQualityFlag.DUPLICATE_SUSPECTED])
                self._log_data_quality_issue('merged_dup_check', idx, df.loc[idx].to_dict(), [DataQualityFlag.DUPLICATE_SUSPECTED])
        return df


    def _process_rent_data(self, rent_alloc: pd.DataFrame, 
                           rent_hist: pd.DataFrame) -> pd.DataFrame:
        logger.info("Processing rent data with budget analysis from Rent Allocation and Rent History...")
        
        df = merge_rent_data(rent_alloc, rent_hist)
        if df.empty:
            logger.warning("Combined rent data is empty. Returning empty DataFrame.")
            essential_cols = ['Date', 'GrossTotal', 'RyanRentPortion', 'TransactionType', 'Payer', 'IsShared', 
        ...
        if "Gross Total" in df.columns: # Ensure this is also mapped if needed by downstream
        ...
        if self.config.RENT_BASELINE > 0:
            variance = (df['GrossTotal'] - self.config.RENT_BASELINE).abs() / self.config.RENT_BASELINE
            high_variance_baseline_mask = variance > self.config.RENT_VARIANCE_THRESHOLD
            for idx in df[high_variance_baseline_mask].index:
                self._update_row_data_quality_flags(df, idx, [DataQualityFlag.RENT_VARIANCE_HIGH])
                self._log_data_quality_issue('rent_baseline_variance', idx, df.loc[idx].to_dict(), [DataQualityFlag.RENT_VARIANCE_HIGH])

        # Check for budget variance issues (from Rent History)
        if 'Budget_Variance_Pct' in df.columns:
            high_variance_budget_mask = df['Budget_Variance_Pct'].abs() > self.config.RENT_BUDGET_VARIANCE_THRESHOLD_PCT
            for idx in df[high_variance_budget_mask].index:
                self._update_row_data_quality_flags(df, idx, [DataQualityFlag.RENT_BUDGET_VARIANCE_HIGH])
                self._log_data_quality_issue('rent_budget_variance', idx, df.loc[idx].to_dict(), [DataQualityFlag.RENT_BUDGET_VARIANCE_HIGH])
    ...
    def _create_rent_audit_note(self, row: pd.Series) -> str:
        ...
        try:
            ...
            return (f"Rent {month_display}: Gross ${gross_total_float:,.2f} paid by {row.get('Payer', 'N/A')}. "
    ...
    def _create_expense_audit_note(self, row: pd.Series) -> str:
        ...
        try:
            ...
            allowed_amount = row.get('AllowedAmount', 0.0) # This should be set before calling
            ...
            elif not row.get('IsShared', False) or abs(allowed_amount_f) < 0.01: # Check IsShared flag
    ...
    def _validate_against_ledger_balance(self, master_ledger: pd.DataFrame, 
                                         transaction_ledger: pd.DataFrame):
        logger.info("Validating master ledger's running balance against transaction ledger's...")
        
        if transaction_ledger.empty or 'Running Balance' not in transaction_ledger.columns or transaction_ledger['Running Balance'].isna().all():
            logger.warning("Transaction ledger is empty or missing valid 'Running Balance' data. Skipping ledger balance validation.")
        ...
        if 'Date of Purchase' in transaction_ledger.columns:
            ...
            ledger_final_balance = ledger_sorted['Running Balance'].iloc[-1] if not ledger_sorted.empty else 0.0
        else: # Fallback if no date column for sorting, just take last row
            ledger_final_balance = transaction_ledger['Running Balance'].iloc[-1] if not transaction_ledger.empty else 0.0
            logger.warning("Transaction Ledger has no 'Date of Purchase' column, using last row for final balance without date sorting.")

        our_final_balance = master_ledger['RunningBalance'].iloc[-1] if not master_ledger.empty and 'RunningBalance' in master_ledger else 0.0
        
        difference = abs(ledger_final_balance - our_final_balance)
        
        if difference > 0.01: # More than 1 cent difference
            msg = (f"Running balance mismatch! Ledger: ${ledger_final_balance:,.2f}, "
                   f"Calculated: ${our_final_balance:,.2f}, Diff: ${difference:,.2f}")
            logger.warning(msg)
            self.validation_results['ledger_balance_match'] = f"Mismatch (Diff: ${difference:,.2f})"
            self._log_data_quality_issue(
                'balance_validation', 'final_balances', 
                {'ledger_balance': ledger_final_balance, 'calculated_balance': our_final_balance, 'difference': difference},
    ...
    def _analyze_rent_budget_variance(self, rent_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'Budget_Variance' not in rent_df.columns or rent_df['Budget_Variance'].isna().all():
            return {'message': 'No budget data available or all variances are NaN'}
        
        variance_data = rent_df[rent_df['Budget_Variance'].notna()].copy() # Work with non-NaN variance data
        
        if not variance_data.empty:
            month_col = 'Month_Display' if 'Month_Display' in variance_data.columns else 'Date' # Prefer Month_Display
            ...
            analysis = {
                'total_budget_variance': round(variance_data['Budget_Variance'].sum(), 2),
                'avg_monthly_variance': round(variance_data['Budget_Variance'].mean(), 2),
                'months_over_budget': len(variance_data[variance_data['Budget_Variance'] > 0.005]), # Small threshold
            ...
            if not variance_data[variance_data['Budget_Variance'] > 0.005].empty:
                analysis['largest_overrun'] = {
                    'month': variance_data.loc[variance_data['Budget_Variance'].idxmax(), month_col],
                    'amount': round(variance_data['Budget_Variance'].max(), 2)
                }
            if not variance_data[variance_data['Budget_Variance'] < -0.005].empty:
                analysis['largest_underrun'] = {
                    'month': variance_data.loc[variance_data['Budget_Variance'].idxmin(), month_col],
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if expense_hist_raw.empty and transaction_ledger_raw.empty:
                logger.error("Both Expense History and Transaction Ledger are empty or failed to load. Cannot proceed.")
                raise ValueError("Critical expense data sources are missing.")
            if rent_alloc_raw.empty:
                logger.warning("Rent Allocation data is empty. Rent-related analysis will be significantly impacted.")


            expense_df = self._process_expense_data(expense_hist_raw, transaction_ledger_raw)
            ...
            if not transaction_ledger_raw.empty: # Only validate if ledger was loaded
                 self._validate_against_ledger_balance(master_ledger, transaction_ledger_raw)
            
            reconciliation_results = self._triple_reconciliation(master_ledger)
            analytics_results = self._perform_advanced_analytics(master_ledger) # This also needs master_ledger
            
            if not rent_df.empty and 'Budget_Variance' in rent_df.columns:
                analytics_results['rent_budget_analysis'] = self._analyze_rent_budget_variance(rent_df)
            
            risk_assessment = self._comprehensive_risk_assessment(master_ledger, analytics_results)
            visualizations = self._create_visualizations_v22(master_ledger, analytics_results, reconciliation_results) # Pass master_ledger
            recommendations = self._generate_recommendations(analytics_results, risk_assessment, reconciliation_results)

            self._validate_results_summary(reconciliation_results, master_ledger) # Renamed to avoid conflict
            ...
            final_results = {
                ...
                'performance_metrics': {
                    ...
                    'total_transactions': len(master_ledger) if not master_ledger.empty else 0
                }
            }
            ...
            if self.data_quality_issues:
                try:
                    ...
                    error_path = output_dir / 'data_quality_issues_PARTIAL_FAILURE_v2.3.csv'
                    error_df.to_csv(error_path, index=False)
                    logger.info(f"Partial data quality issues logged to {error_path}")
                except Exception as log_e:
                    logger.error(f"Could not write partial data quality log during failure: {log_e}")
            raise
    ...
    def _explode_audit(self, audit_note: str) -> Tuple[str, str, str, str]:
        ...
        if "FULLY SHARED" in audit_str: share_type = "FULLY_SHARED"
        elif "PARTIALLY SHARED" in audit_str: share_type = "PARTIALLY_SHARED"
        elif "PERSONAL EXPENSE" in audit_str: share_type = "PERSONAL"
        else: share_type = "SHARED" if "share" in audit_str.lower() else "<NA>"
    ...
    def _create_master_ledger(self, rent_df: pd.DataFrame, expense_df: pd.DataFrame) -> pd.DataFrame:
        logger.info("Creating master ledger v2.3 from processed rent and expense data...")
        ...
        for df_iter, name in [(rent_df, "rent_df"), (expense_df, "expense_df")]:
            if df_iter.empty:
                logger.warning(f"{name} is empty. It will not contribute to master ledger.")
            ...
            for col in common_cols:
                if col not in df_iter.columns:
                    logger.warning(f"Column '{col}' missing in {name}. Adding as NaN/0/False.")
        ...
        dfs_to_concat = []
        if not rent_df.empty: dfs_to_concat.append(rent_df[common_cols])
        if not expense_df.empty: dfs_to_concat.append(expense_df[common_cols])

        if not dfs_to_concat:
            logger.error("Both processed rent and expense DataFrames are empty. Master ledger cannot be created.")
            return pd.DataFrame(columns=common_cols + ['RunningBalance', 'TransactionID', 'DataLineage', 
        ...
        if master['Date'].isna().any():
            logger.warning(f"{master['Date'].isna().sum()} transactions have missing dates in master ledger. They will be sorted to the beginning or end.")
            # Optionally, impute again or drop:
            # master = master.dropna(subset=['Date'])

        master = master.sort_values(by='Date', ascending=True, na_position='first').reset_index(drop=True)
        ...
        master['DataLineage'] = master.apply(
            lambda row: f"Source: {row.get('TransactionType','NA')} | OriginalIndex(PostProc): {row.name} | Processing: v2.3", axis=1
        )
        
        # Audit split columns
        audit_components = master['AuditNote'].astype(str).apply(self._explode_audit) # Ensure AuditNote is str
        master[['Who_Paid_Text', 'Share_Type', 'Shared_Reason', 'DataQuality_Audit']] = pd.DataFrame(
            audit_components.tolist(), index=master.index
        )
        ...
        if not master.empty and master['Date'].notna().any():
            ...
            logger.warning("Master ledger is empty or contains no valid dates after processing.")
        return master

    def _generate_transaction_id(self, row: pd.Series) -> str:
        """Generate unique transaction ID. Copied from original."""
        date_str = row['Date'].isoformat() if isinstance(row.get('Date'), (datetime, pd.Timestamp)) and pd.notna(row.get('Date')) else str(row.get('Date', 'NoDate'))
        # Use .get for safety, provide defaults
        key_data = (f"{date_str}_"
    ...
    def _triple_reconciliation(self, master_ledger: pd.DataFrame) -> Dict[str, Any]:
        ...
        if master_ledger.empty or 'BalanceImpact' not in master_ledger.columns :
            logger.warning("Master ledger is empty or missing 'BalanceImpact'. Reconciliation will yield zero values.")
            return {
                'method1_running_balance': 0, 'method2_variance_ryan_vs_fair': 0, 'method3_category_sum_ryan_vs_fair': 0,
                'reconciled': True, 'max_difference': 0, 'total_shared_amount': 0,
                'ryan_total_fair_share': 0, 'jordyn_total_fair_share': 0, 
                'ryan_actually_paid_for_shared': 0, 'jordyn_actually_paid_for_shared': 0,
                'ryan_net_variance_from_fair_share': 0, 'jordyn_net_variance_from_fair_share': 0,
                'category_details': [], 'final_balance_reported': 0, 
                'who_owes_whom': 'No transactions', 'amount_owed': 0
            }
        ...
        if shared_only.empty: # If no shared transactions, M2 and M3 are zero.
            ...
            ryan_actually_paid_for_shared = shared_only[shared_only['Payer'].str.lower() == 'ryan']['AllowedAmount'].sum()
            jordyn_actually_paid_for_shared = shared_only[shared_only['Payer'].str.lower() == 'jordyn']['AllowedAmount'].sum()
            ...
            for trans_type in ['RENT', 'EXPENSE']: # Assuming these are the main types
                ...
                if not type_data.empty:
                    ...
                    type_ryan_paid_cat = type_data[type_data['Payer'].str.lower() == 'ryan']['AllowedAmount'].sum()
                    type_jordyn_paid_cat = type_data[type_data['Payer'].str.lower() == 'jordyn']['AllowedAmount'].sum()
        ...
        who_owes = "Ryan owes Jordyn" if final_balance_to_report > 0.005 else \
                   "Jordyn owes Ryan" if final_balance_to_report < -0.005 else "Settled"
        amount_owed_val = abs(final_balance_to_report)

        logger.info(f"Triple Reconciliation Results (v2.3 logic):")
        logger.info(f"  M1 (Running Balance): ${method1_balance:,.2f} ({'Ryan owes Jordyn' if method1_balance > 0 else 'Jordyn owes Ryan' if method1_balance < 0 else 'Settled'})")
        logger.info(f"  M2 (Ryan's Net Payment vs Fair Share): ${method2_balance:,.2f} ({'Jordyn owes Ryan' if method2_balance > 0 else 'Ryan owes Jordyn' if method2_balance < 0 else 'Settled'})")
        logger.info(f"  M3 (Category Sum of Ryan's Net Payment vs Fair Share): ${method3_balance:,.2f} ({'Jordyn owes Ryan' if method3_balance > 0 else 'Ryan owes Jordyn' if method3_balance < 0 else 'Settled'})")
        logger.info(f"  All Reconciled (M1 ~ -M2, M2 ~ M3): {all_reconciled}")
        if not all_reconciled:
            logger.error(f"Reconciliation discrepancy! M1+M2 diff: {method1_balance + method2_balance:.4f}, M2-M3 diff: {method2_balance - method3_balance:.4f}")
        ...
        return {
            ...
            'ryan_net_variance_from_fair_share': round(method2_balance, 2), # M2 is this value
            'jordyn_net_variance_from_fair_share': round((jordyn_actually_paid_for_shared - jordyn_total_fair_share), 2),
    ...
    def _perform_advanced_analytics(self, master_ledger: pd.DataFrame) -> Dict[str, Any]:
        ...
        if valid_dates_ledger.empty:
            logger.warning("Master ledger has no valid dates after coercion for advanced analytics.")
        ...
        if not shared_ledger.empty:
            ...
            monthly_cash_flow = shared_ledger.groupby(
                [pd.Grouper(key='Date', freq='ME'), 'Payer']
            )['AllowedAmount'].sum().unstack(fill_value=0).round(self.config.CURRENCY_PRECISION)
            analytics['monthly_payments_by_payer_for_shared_items'] = monthly_cash_flow.to_dict('index') # More JSON friendly
        else:
        ...
        if 'RunningBalance' in valid_dates_ledger.columns and 'TransactionID' in valid_dates_ledger.columns:
            high_balance_transactions = valid_dates_ledger[valid_dates_ledger['RunningBalance'].abs() > self.config.LIQUIDITY_STRAIN_THRESHOLD]
        ...
        expense_only_df = valid_dates_ledger[(valid_dates_ledger['TransactionType'] == 'EXPENSE') & (valid_dates_ledger['IsShared'] == True)].copy()
        if not expense_only_df.empty and 'Merchant' in expense_only_df.columns:
            ...
            if not category_summary.empty:
                ...
                analytics['expense_category_analysis'] = {
                    'summary_table': category_summary.reset_index().to_dict('records'), # More JSON friendly
                    'pareto_80_categories_list': pareto_80_categories,
                    'pareto_80_amount_sum': round(category_totals.loc[pareto_80_categories].sum(),2) if pareto_80_categories else 0.0
                }
        else:
        ...
        if len(monthly_shared_spending) >= 3:
            ...
            analytics['monthly_shared_spending_trend'] = {
                ...
                'monthly_values': {d.strftime('%Y-%m'): round(val,2) for d, val in monthly_shared_spending.items()}
            }
            # Forecast
            ...
            if not monthly_shared_spending.index.empty:
                ...
                analytics['monthly_shared_spending_forecast'] = {'message': "Not enough data for forecast (source index empty)."}
        ...
        if 'RunningBalance' in valid_dates_ledger.columns:
            ...
            if not month_end_balances.empty:
             ...
             analytics['month_end_running_balance_stats'] = {'message': "RunningBalance column not available."}
    ...
    def _comprehensive_risk_assessment(self, master_ledger: pd.DataFrame, analytics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        risks: Dict[str, Any] = {'overall_risk_level': 'LOW', 'details': []} # Ensure type for details
        ...
        if 'potential_liquidity_strain_points' in analytics and analytics['potential_liquidity_strain_points']:
            ...
            risks['details'].append({
                ...
                'level': 'MEDIUM' if strain_count > 0 else 'LOW' # Should this be HIGH if many?
            })

        trend_info = analytics.get('monthly_shared_spending_trend', {})
        if trend_info.get('slope_per_month', 0) > 100 and trend_info.get('trend_significant', False): # Example threshold
            risks['details'].append({
        ...
        if 'total_budget_variance' in rent_budget_analysis:
            if abs(rent_budget_analysis.get('avg_monthly_variance',0)) > 200 : # Example: if avg variance is high
                 risks['details'].append({
        ...
        if self.validation_results.get('ledger_balance_match') not in ["Matched", "Skipped - No Ledger Data", None]:
             risks['details'].append({
    ...
    def _create_visualizations_v22(self, master_ledger: pd.DataFrame, analytics: Dict[str, Any], reconciliation: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            path, alt = self._build_running_balance_timeline(master_ledger.copy(), output_dir) # Pass copy
            viz_paths['running-balance-timeline'] = str(path)
            self.alt_texts['running-balance-timeline'] = alt
        except Exception as e: logger.error(f"Failed running balance timeline: {e}", exc_info=True)

        # 2. Waterfall Category Impact
        try:
            path, alt = self._build_waterfall_category_impact(master_ledger.copy(), output_dir, theme)
            viz_paths['waterfall-category-impact'] = str(path)
            self.alt_texts['waterfall-category-impact'] = alt
        except Exception as e: logger.error(f"Failed waterfall chart: {e}", exc_info=True)
        
        # 3. Monthly Shared Trend
        try:
            path, alt = self._build_monthly_shared_trend(master_ledger.copy(), analytics, output_dir)
            viz_paths['monthly-shared-trend'] = str(path)
            self.alt_texts['monthly-shared-trend'] = alt
        except Exception as e: logger.error(f"Failed monthly trend: {e}", exc_info=True)

        # 4. Liquidity Heatmap (Payer vs Transaction Type)
        try:
            path, alt = self._build_payer_type_heatmap(master_ledger.copy(), output_dir, theme) # Renamed for clarity
            viz_paths['payer-type-heatmap'] = str(path)
            self.alt_texts['payer-type-heatmap'] = alt
        except Exception as e: logger.error(f"Failed payer-type heatmap: {e}", exc_info=True)

        # 5. Calendar Heatmaps
        try:
            calendar_paths = self._build_calendar_heatmaps(master_ledger.copy(), output_dir)
        ...
        except Exception as e: logger.error(f"Failed calendar heatmaps: {e}", exc_info=True)

        # 6. Treemap Shared Spending
        try:
            path, alt = self._build_treemap_shared_spending(master_ledger.copy(), output_dir, theme) # Removed analytics dep
            viz_paths['treemap-shared-spending'] = str(path)
            self.alt_texts['treemap-shared-spending'] = alt
        except Exception as e: logger.error(f"Failed treemap: {e}", exc_info=True)
        ...
        except Exception as e: logger.error(f"Failed anomaly scatter: {e}", exc_info=True)

        # 8. Pareto Concentration
        try:
            path, alt = self._build_pareto_concentration(master_ledger.copy(), output_dir) # Removed analytics dep
            viz_paths['pareto-concentration'] = str(path)
            self.alt_texts['pareto-concentration'] = alt
        except Exception as e: logger.error(f"Failed pareto chart: {e}", exc_info=True)

        # 9. Sankey Settlements
        try:
            path, alt = self._build_sankey_settlements(master_ledger.copy(), output_dir, theme)
            viz_paths['sankey-settlements'] = str(path)
            self.alt_texts['sankey-settlements'] = alt
        except Exception as e: logger.error(f"Failed sankey diagram: {e}", exc_info=True)
        
        # 10. Data Quality Table
        try:
            path, alt = self._build_data_quality_table_viz(master_ledger.copy(), output_dir, theme) # Renamed for clarity
            viz_paths['data-quality-table-viz'] = str(path)
            self.alt_texts['data-quality-table-viz'] = alt
        except Exception as e: logger.error(f"Failed data quality table viz: {e}", exc_info=True)
    ...
    def _build_running_balance_timeline(self, ledger_df: pd.DataFrame, output_dir: Path) -> Tuple[Path, str]:
        ledger_df = ledger_df.dropna(subset=['Date', 'RunningBalance'])
        if ledger_df.empty: return output_dir / 'no_data.png', "No data for running balance timeline."
        ...
        if pd.notna(min_bal) and pd.notna(max_bal):
            ax.axhspan(-self.config.LIQUIDITY_STRAIN_THRESHOLD, self.config.LIQUIDITY_STRAIN_THRESHOLD, 
                       alpha=0.05, color='green', label='Normal Range')
            ax.axhspan(self.config.LIQUIDITY_STRAIN_THRESHOLD, max_bal + 1000, 
                       alpha=0.15, color='red', label='Ryan Liquidity Strain Potential') # Jordyn owes Ryan more
            ax.axhspan(min_bal - 1000, -self.config.LIQUIDITY_STRAIN_THRESHOLD, 
                       alpha=0.15, color='orange', label='Jordyn Liquidity Strain Potential') # Ryan owes Jordyn more
        
        ax.axhline(0, color='grey', linestyle='--', linewidth=0.8, alpha=0.5)
        
        # Annotate largest impacts
        if 'BalanceImpact' in ledger_df.columns:
            top5 = ledger_df.iloc[ledger_df['BalanceImpact'].abs().nlargest(5).index] # Use .iloc with index
            for _, row in top5.iterrows():
                if pd.notna(row['Date']) and pd.notna(row['RunningBalance']): # Check for NaT/NaN
                    desc_short = str(row.get('Description',''))[:20] + '...' if len(str(row.get('Description',''))) > 20 else str(row.get('Description',''))
                    ax.annotate(f"{desc_short}\n${row['BalanceImpact']:,.0f}",
    ...
    def _build_waterfall_category_impact(self, ledger_df: pd.DataFrame, output_dir: Path, theme) -> Tuple[Path, str]:
        ...
        ledger_df['MerchantCategory'] = ledger_df.apply(
            ...
            else self._categorize_merchant(row.get('Merchant', 'Other')), axis=1
        )
        category_impacts = ledger_df.groupby('MerchantCategory')['BalanceImpact'].sum().sort_values(ascending=False)
        if category_impacts.empty : return output_dir / 'no_data.html', "No category impacts for waterfall."
        ...
        fig = go.Figure(go.Waterfall(
            name = "Balance Impact",
            orientation = "v",
            measure = ["relative"] * (len(x_labels) -1) + ["total"] if x_labels else [], # last one is total
            x = x_labels,
            textposition = "outside",
            text = [f"${val:,.0f}" for val in y_values],
            y = y_values,
            connector = {"line":{"color":"rgb(63, 63, 63)"}},
            increasing = {"marker":{"color":TABLEAU_COLORBLIND_10[1]}}, # Green for positive impact (Ryan gets money/owes less)
            decreasing = {"marker":{"color":TABLEAU_COLORBLIND_10[0]}}, # Red for negative impact (Ryan pays/owes more)
    ...
    def _build_monthly_shared_trend(self, ledger_df: pd.DataFrame, analytics: Dict[str, Any], output_dir: Path) -> Tuple[Path, str]:
        ...
        if len(monthly_shared_series) < 3:
            return output_dir / 'no_data.png', "Insufficient data for trend plot (need at least 3 months)."

        fig, ax = plt.subplots(figsize=(12, 7))
        x_labels = [d for d in monthly_shared_series.index] # Dates are already YYYY-MM strings
        ...
        if len(x_pos) >=2 : # Need at least 2 points for linregress for plotting
            current_slope, intercept, _, _, _ = stats.linregress(x_pos, monthly_shared_series.values)
            regression_line = current_slope * x_pos + intercept
            ax.plot(x_pos, regression_line, color=TABLEAU_COLORBLIND_10[0], linewidth=2, 
    ...
    def _build_payer_type_heatmap(self, ledger_df: pd.DataFrame, output_dir: Path, theme) -> Tuple[Path, str]:
        ...
        if shared_df.empty: return output_dir / 'no_data.html', "No shared transactions for payer-type heatmap."

        pivot_data = shared_df.pivot_table(
            values='AllowedAmount', index='Payer', columns='TransactionType',
            aggfunc='sum', fill_value=0
        )
        if pivot_data.empty: return output_dir / 'no_data.html', "Pivot data empty for heatmap."

        fig = go.Figure(data=go.Heatmap(
            z=pivot_data.values, x=pivot_data.columns, y=pivot_data.index,
            colorscale='Blues', text=[[f'${val:,.0f}' for val in row] for row in pivot_data.values],
    ...
    def _build_calendar_heatmaps(self, ledger_df: pd.DataFrame, output_dir: Path) -> Dict[str, Tuple[Path, str]]:
        calendar_paths = {}
        ...
        if shared_only.empty: return calendar_paths
        ...
        for year_month_period in shared_only['YearMonth'].unique():
            ...
            daily_spending.index = pd.to_datetime(daily_spending.index) # Ensure datetime index
            ...
            ax.xaxis.set_major_formatter(plt.FixedFormatter(all_days_in_month.index.strftime('%d'))) # Show day number
            plt.tight_layout()
    ...
    def _build_treemap_shared_spending(self, ledger_df: pd.DataFrame, output_dir: Path, theme) -> Tuple[Path, str]:
        ...
        if shared_df.empty: return output_dir / 'no_data.html', "No shared transactions for treemap."

        category_data = []
        # Rent
        rent_total = shared_df[shared_df['TransactionType'] == 'RENT']['AllowedAmount'].sum()
        if rent_total > 0:
            category_data.append({'Category': 'RENT', 'Amount': rent_total, 'Parent': ''})
        ...
        if not expense_df.empty and 'Merchant' in expense_df.columns:
            expense_df['Category'] = expense_df['Merchant'].apply(self._categorize_merchant)
            category_totals = expense_df.groupby('Category')['AllowedAmount'].sum()
            
            for cat, amount in category_totals.items():
                if amount > 0:
                    category_data.append({'Category': cat, 'Amount': amount, 'Parent': 'EXPENSES'})
            
            total_expenses = category_totals.sum()
            if total_expenses > 0: # Add parent node for expenses if any exist
                category_data.append({'Category': 'EXPENSES', 'Amount': total_expenses, 'Parent': ''})
        
        if not category_data: return output_dir / 'no_data.html', "No categories for treemap."
        ...
        fig = go.Figure(go.Treemap(
            ...
            marker_colorscale='Blues', line=dict(width=1) # Simplified marker
        ))
    ...
    def _build_anomaly_scatter(self, ledger_df: pd.DataFrame, output_dir: Path) -> Tuple[Path, str]:
        ...
        if plot_df.empty: return output_dir / 'no_data.png', "No valid data points for anomaly scatter plot."
        ...
        for flag_val in unique_flags:
            ...
            ax.scatter(subset['AllowedAmount'], subset['BalanceImpact'], 
                       label=flag_val, alpha=0.6, s=50, color=flag_colors[flag_val])
        ...
        if not plot_df.empty and 'BalanceImpact' in plot_df.columns:
            ...
            if len(plot_df['BalanceImpact'].abs().dropna()) > 0 :
                threshold_value = np.percentile(plot_df['BalanceImpact'].abs().dropna(), threshold_pct)
                ...
                for _, row in outliers.iterrows():
                    ...
                    ax.annotate(desc_short, xy=(row['AllowedAmount'], row['BalanceImpact']),
    ...
    def _build_pareto_concentration(self, ledger_df: pd.DataFrame, output_dir: Path) -> Tuple[Path, str]:
        if ledger_df.empty or 'AllowedAmount' not in ledger_df.columns:
            return output_dir / 'no_data.png', "No data for Pareto chart."

        shared_only = ledger_df[ledger_df['IsShared'] == True].copy()
        if shared_only.empty: return output_dir / 'no_data.png', "No shared transactions for Pareto chart."
        ...
        for i, val in enumerate(category_totals.values):
            ax1.text(x_pos[i], val + (category_totals.values.max()*0.02), f'${val:,.0f}', ha='center', va='bottom', fontsize=9, rotation=30) # Adjust offset
        ...
        if len(idx_80) > 0:
            ...
            ax2.annotate(f'{x_80 + 1} cat.\nreach 80%', xy=(x_80, y_80), xytext=(x_80 + 0.5, y_80 - 10),
        ...
        ax2.legend(loc='center right'); plt.tight_layout()
        path = output_dir / 'pareto_concentration.png'
        plt.savefig(path, dpi=300, bbox_inches='tight'); plt.close(fig)
        alt_text = f"Pareto chart of shared spending concentration."
        return path, alt_text

    def _build_sankey_settlements(self, ledger_df: pd.DataFrame, output_dir: Path, theme) -> Tuple[Path, str]:
        if ledger_df.empty: return output_dir / 'no_data.html', "No data for Sankey diagram."
        ...
        labels = ["Ryan Pays", "Jordyn Pays", "Shared Expense Pool", 
                  "Ryan's Fair Share", "Jordyn's Fair Share", 
                  "Ryan Final Position", "Jordyn Final Position"]
        
        source_indices, target_indices, values, link_labels = [], [], [], []

        # Flows into Shared Expense Pool
        if ryan_paid_shared > 0:
            source_indices.append(0); target_indices.append(2); values.append(ryan_paid_shared); link_labels.append(f"${ryan_paid_shared:,.0f}")
        if jordyn_paid_shared > 0:
            source_indices.append(1); target_indices.append(2); values.append(jordyn_paid_shared); link_labels.append(f"${jordyn_paid_shared:,.0f}")

        # Flows from Shared Pool to Fair Shares
        if ryan_fair_share > 0: # From Shared Pool to Ryan's Fair Share
            source_indices.append(2); target_indices.append(3); values.append(ryan_fair_share); link_labels.append(f"Ryan's Share ${ryan_fair_share:,.0f}")
        if jordyn_fair_share > 0: # From Shared Pool to Jordyn's Fair Share
            source_indices.append(2); target_indices.append(4); values.append(jordyn_fair_share); link_labels.append(f"Jordyn's Share ${jordyn_fair_share:,.0f}")
    ...
    def _build_data_quality_table_viz(self, ledger_df: pd.DataFrame, output_dir: Path, theme) -> Tuple[Path, str]:
        ...
        if 'BalanceImpact' in ledger_df.columns and not ledger_df.empty:
            ...
            if not valid_impacts.empty and len(valid_impacts) > 10: # Need enough data for percentile
        ...
        if combined.empty: return output_dir / 'no_data.html', "No data quality issues or high impact transactions found for table."

        display_cols = ['Date', 'TransactionType', 'Payer', 'Description', 
                        'ActualAmount', 'AllowedAmount', 'BalanceImpact', 'DataQualityFlag']
        ...
        fig.update_layout(title=f'Data Quality Issues & High-Impact Transactions ({len(display_df)} rows)', template=theme, height=600)
    ...
    def _generate_recommendations(self, analytics: Dict[str, Any], risk_assessment: Dict[str, Any], reconciliation: Dict[str, Any]) -> List[str]:
        ...
        for risk_detail in risk_assessment.get('details', []):
            if risk_detail.get('level') in ['HIGH', 'MEDIUM']:
                recommendations.append(f"Risk Alert ({risk_detail.get('risk_type', 'Risk')} - {risk_detail.get('level')}): {risk_detail.get('assessment', '')}")

        trend_info = analytics.get('monthly_shared_spending_trend', {})
        if trend_info.get('slope_per_month', 0) > 100 and trend_info.get('trend_significant', False): # Example
            recommendations.append(
        ...
        rent_budget_analysis = analytics.get('rent_budget_analysis',{})
        if 'avg_monthly_variance' in rent_budget_analysis and abs(rent_budget_analysis.get('avg_monthly_variance',0)) > 50 : # Example
             recommendations.append(
        ...
        if not recommendations:
            recommendations.append("No specific high-priority recommendations at this time. Financials appear stable. Continue good practices.")
    ...
    def _calculate_data_quality_score(self, master_ledger: pd.DataFrame) -> float:
        ...
        if master_ledger.empty or 'DataQualityFlag' not in master_ledger.columns: return 0.0
        ...
        for flags_str_per_row in master_ledger['DataQualityFlag'].dropna():
            if flags_str_per_row != DataQualityFlag.CLEAN.value:
                ...
                for flag in individual_flags_in_row:
                    if flag and flag != DataQualityFlag.CLEAN.value: # Ensure flag is not empty string
                        flag_counts[flag] = flag_counts.get(flag, 0) + 1
        return flag_counts if flag_counts else {"All Clear": len(master_ledger)}


    def _validate_results_summary(self, reconciliation: Dict[str, Any], master_ledger: pd.DataFrame) -> None:
        ...
        if not master_ledger.empty and 'BalanceImpact' in master_ledger.columns and 'RunningBalance' in master_ledger.columns:
            # Ensure columns are numeric and not all NaN before sum/iloc
            if master_ledger['BalanceImpact'].notna().any() and master_ledger['RunningBalance'].notna().any():
                sum_balance_impacts = master_ledger['BalanceImpact'].sum()
                final_running_balance = master_ledger['RunningBalance'].iloc[-1]
                self.validation_results['balance_impact_sum_vs_running_balance'] = abs(sum_balance_impacts - final_running_balance) < 0.015 # Small tolerance
            else:
                self.validation_results['balance_impact_sum_vs_running_balance'] = "Skipped - NaN in financial columns"
        else: # If ledger empty or columns missing
            self.validation_results['balance_impact_sum_vs_running_balance'] = True # Or "Skipped"
        ...
        if 'Who_Paid_Text' in master_ledger.columns: # Check if audit columns were created
            self.validation_results['audit_columns_exist'] = all(col in master_ledger.columns 
                                                                for col in ['Who_Paid_Text', 'Share_Type', 
                                                                           'Shared_Reason', 'DataQuality_Audit'])
        else:
            self.validation_results['audit_columns_exist'] = False


        all_passed_numeric = sum(1 for v in self.validation_results.values() if isinstance(v, bool) and v)
        total_numeric_checks = sum(1 for v in self.validation_results.values() if isinstance(v, bool))
        
        logger.info(f"Validation checks completed. {all_passed_numeric}/{total_numeric_checks} boolean checks passed.")
        for k,v in self.validation_results.items():
            logger.info(f"  Validation - {k}: {v}")
        
        if not all(v for v in self.validation_results.values() if isinstance(v, bool)): # If any boolean check is False
            failed_checks = [k for k, v in self.validation_results.items() if isinstance(v, bool) and not v]
            logger.error(f"Some critical validations FAILED: {', '.join(failed_checks)}")
    ...
    def _categorize_merchant(self, merchant: Optional[str]) -> str:
        ...
        categories = {
            'Groceries': ['fry', 'safeway', 'walmart', 'target', 'costco', 'trader joe', 'whole foods', 'kroger', 'albertsons', 'grocery', 'sprouts'],
            'Utilities': ['electric', 'gas', 'water', 'internet', 'phone', 'cox', 'srp', 'aps', 'centurylink', 'utility', 'conservice', 'google fi', 't-mobile', 'verizon'],
            'Dining Out': ['restaurant', 'cafe', 'coffee', 'starbucks', 'pizza', 'sushi', 'mcdonald', 'chipotle', 'subway', 'doordash', 'grubhub', 'uber eats', 'postmates', 'culinary dropout', 'bar'],
            'Transport': ['uber', 'lyft', 'gas station', 'shell', 'chevron', 'circle k', 'qt', 'auto repair', 'fuel', 'parking', 'toll', 'bird', 'lime', 'waymo'],
            'Entertainment': ['movie', 'theater', 'netflix', 'spotify', 'hulu', 'disney', 'concert', 'event', 'game', 'amc', 'cinemark', 'ticketmaster', 'steam'],
            'Healthcare': ['pharmacy', 'cvs', 'walgreens', 'doctor', 'medical', 'dental', 'clinic', 'hospital', 'optometrist', 'vision'],
            'Shopping': ['amazon', 'best buy', 'macys', 'nordstrom', 'online store', 'retail', 'clothing', 'electronics', 'home goods', 'ikea'],
            'Travel': ['airline', 'hotel', 'airbnb', 'expedia', 'booking.com', 'southwest', 'delta', 'united'],
            'Services': ['haircut', 'gym', 'consulting', 'legal', 'accounting', 'cleaning'],
            'Rent': ['rent', 'property management', 'landlord'] # Though rent is usually its own TransactionType
        }
    ...
    def _generate_outputs(self, master_ledger: pd.DataFrame, reconciliation: Dict[str, Any],
        ...
        master_ledger_export['Category_Display'] = master_ledger_export['TransactionType'] # Basic category
        master_ledger_export['Amount_Charged_Display'] = master_ledger_export['ActualAmount']
        master_ledger_export['Shared_Amount_Display'] = master_ledger_export['AllowedAmount']
        master_ledger_export['Cumulative_Balance_Display'] = master_ledger_export['RunningBalance']
        ...
        if not master_ledger_export.empty:
            ...
            logger.info("DEBUG: Coffee transaction in master_ledger_export before CSV write:")
            coffee_debug_pre_csv = master_ledger_export[master_ledger_export['Description'].str.contains("coffee", na=False, case=False)]
            if not coffee_debug_pre_csv.empty:
                logger.info(coffee_debug_pre_csv[['Date', 'Description', 'ActualAmount', 'AllowedAmount', 'Shared_Amount_Display']].to_string())
            else:
                logger.info("DEBUG: Coffee transaction not found in master_ledger_export before CSV write.")

            master_ledger_export.to_csv(ledger_path, index=False) # date_format not needed if already string
            ...
            recon_cols = ['Date', 'Category_Display', 'Payer', 'Description', 'Amount_Charged_Display', 
        ...
        summary_data = {
            ...
            "Triple Reconciliation Matched": "YES" if reconciliation.get('reconciled', False) else "NO",
            "Ledger Balance Matched": str(self.validation_results.get('ledger_balance_match', 'N/A')),
        ...
        if 'data_sources_summary' in analytics: # It was put in final_results, not analytics directly
            src_summary = self.analyze_results.get('data_sources_summary', {}) if hasattr(self, 'analyze_results') else {} # Access through attribute if needed
        ...
        with open(alt_text_path, 'w') as f: json.dump(self.alt_texts, f, indent=2)
        ...
        try:
            with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
                ...
                if not master_ledger_export.empty:
                    ...
                    excel_ledger_cols = [col for col in final_recon_cols if col in master_ledger_export.columns] # Use recon_cols as a base
                    master_ledger_export[excel_ledger_cols].to_excel(writer, sheet_name='Master Ledger Highlights', index=False)
                
                pd.DataFrame(list(reconciliation.items()), columns=['Metric', 'Value']).to_excel(writer, sheet_name='Reconciliation Details', index=False)
                ...
                if 'expense_category_analysis' in analytics and isinstance(analytics['expense_category_analysis'].get('summary_table'),list):
                     pd.DataFrame(analytics['expense_category_analysis']['summary_table']).to_excel(writer, sheet_name='Expense Category Stats', index=False)
                ...
                visual_index_data = [{'Visualization': k, 'Filename': Path(v).name, 'Alt Text': self.alt_texts.get(k, 'N/A')} for k,v in visualizations.items()]
        ...
        except Exception as e_excel: logger.error(f"Failed to generate Excel report: {e_excel}", exc_info=True)

        # 7. Dashboard HTML
        try:
            dashboard_path = self._generate_dashboard_html(visualizations, output_dir) # Renamed
            output_paths['dashboard'] = str(dashboard_path)
            logger.info(f"Saved: {dashboard_path}")
        except Exception as e_dash: logger.error(f"Failed to generate dashboard: {e_dash}", exc_info=True)

        # 8. Executive PDF
        try:
            pdf_path = self._generate_executive_summary_pdf(summary_data, visualizations, recommendations, output_dir) #Renamed
            output_paths['executive_pdf'] = str(pdf_path)
            logger.info(f"Saved: {pdf_path}")
        except Exception as e_pdf: logger.error(f"Failed to generate executive PDF: {e_pdf}", exc_info=True)

        logger.info(f"Generated {len(output_paths)} output files in {output_dir.resolve()}")
        return output_paths

    def _generate_dashboard_html(self, visualizations: Dict[str, str], output_dir: Path) -> Path:
...
"""
        ...
        with open(dashboard_path, 'w', encoding='utf-8') as f: f.write(html_content)
        return dashboard_path

    def _generate_executive_summary_pdf(self, summary_data: Dict[str, str], visualizations: Dict[str, str], 
                                     recommendations: List[str], output_dir: Path) -> Path:
        ...
        doc = SimpleDocTemplate(str(pdf_path), pagesize=landscape(A4), topMargin=0.5*inch, bottomMargin=0.5*inch, leftMargin=0.5*inch, rightMargin=0.5*inch)
        story = []
        styles = getSampleStyleSheet()
        styles.add(ParagraphStyle(name='Justify', alignment=TA_LEFT)) # TA_LEFT for normal text
        ...
        styles.add(ParagraphStyle(name='H2Custom', fontSize=16, leading=18, alignment=TA_LEFT, spaceAfter=10, textColor=colors.HexColor('#005A99')))
        ...
        metrics_table_data = [['Metric', 'Value']] + [[k,str(v)] for k,v in summary_data.items()] # Ensure v is str
        
        # Create table with specific column widths
        table_width = A4[1] - 1*inch # landscape A4 width minus margins
        col_widths = [table_width * 0.4, table_width * 0.6] # 40% for metric, 60% for value
        
        metrics_table = Table(metrics_table_data, colWidths=col_widths)
        metrics_table.setStyle(TableStyle([
            ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#007ACC')), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke),
        ...
        for i, rec in enumerate(recommendations,1): story.append(Paragraph(f"{i}. {rec}", styles['Normal']))
        story.append(PageBreak())
        ...
        for viz_name, viz_path_str in sorted(visualizations.items()):
            ...
            if viz_path.suffix == '.png': # Only embed PNGs in PDF
                ...
                try:
                    ...
                    story.append(Paragraph(f"[Could not load image: {viz_path.name} - {img_err}]", styles['Italic']))

                alt = self.alt_texts.get(viz_name, '')
                if alt: story.append(Paragraph(f"<i>{alt}</i>", styles['Italic']))
                story.append(Spacer(1, 0.2*inch))
        ...
        try:
            ...
            with open(fallback_path, "w") as f:
                ...
                for k,v in summary_data.items(): f.write(f"{k}: {v}\n")
                f.write("\nRecommendations:\n")
...
class TestEnhancedAnalyzer(unittest.TestCase):
    def setUp(self):
        ...
        exp_fixture_header = ['Date of Purchase', 'Name', 'Actual Amount', 'Allowed Amount', 'Merchant', 'Description']
        exp_data_list = [
            ['Junk Row 1', 'Junk', 'Junk', 'Junk', 'Junk', 'Junk'], # Simulating a non-header row
            ...
            ['01/15/2024', 'Ryan', '$20.00', '', 'Coffee', 'Morning coffee 2x to calculate'] # Test blank allowed
        ]
        pd.DataFrame(exp_data_list).to_csv(self.expense_fixture_path, index=False, header=False)

        self.ledger_fixture_path = self.test_dir / "test_ledger.csv"
        # Adjusted to list of lists for precise CSV structure control
        # load_transaction_ledger expects headers at raw.iloc[1]
        ledger_fixture_header = ['Date of Purchase', 'Name', 'Actual Amount', 'Running Balance', 'Merchant', 'Description']
        ledg_data_list = [
            ['Some Title or Junk Row 0', None, None, None, None, None], # This is raw.iloc[0]
            ledger_fixture_header, # This is raw.iloc[1], used as headers by load_transaction_ledger
            ['01/05/2024', 'Ryan', '$100.00', '$100.00', 'Groceries', 'Ledger Groceries'],
            ['01/10/2024', 'Jordyn', '$50.00', '$150.00', 'Gas', 'Ledger Gas']
        ]
        pd.DataFrame(ledg_data_list).to_csv(self.ledger_fixture_path, index=False, header=False)
    ...
    def tearDown(self):
        ...
        for p in [self.expense_fixture_path, self.ledger_fixture_path, 
                  self.rent_alloc_fixture_path, self.rent_hist_fixture_path]:
        ...
        if log_file.exists(): 
            try:
                ...
                logger.warning(f"Could not delete {log_file} during teardown, likely still in use.")

        output_dir = Path('analysis_output')
        if output_dir.exists():
            for item in output_dir.iterdir(): item.unlink(missing_ok=True) # Delete files in output
            try:
                output_dir.rmdir() # Delete directory if empty
            except OSError:
                 logger.warning(f"Could not remove output_dir {output_dir} during teardown, it might not be empty.")

        if self.test_dir.exists():
            try:
                self.test_dir.rmdir()
            except OSError:
                logger.warning(f"Could not remove test_dir {self.test_dir} during teardown, it might not be empty.")
    ...
    def test_full_analysis_run_v23_fixtures(self):
        ...
        analyzer = EnhancedSharedExpenseAnalyzer(
            self.expense_fixture_path, self.ledger_fixture_path,
            self.rent_alloc_fixture_path, self.rent_hist_fixture_path,
            self.config
        )
        results = {}
        try:
            ...
            self.assertIn('master_ledger', results['output_paths']) # Check specific output

            # Basic checks
            self.assertGreater(results['data_sources_summary']['expense_history']['rows'], 0)
            self.assertGreater(results['data_sources_summary']['rent_allocation']['rows'], 0)
            
            # Check 2x calculation from expense fixture
            master_ledger_df = pd.read_csv(results['output_paths']['master_ledger'])
            coffee_transaction = master_ledger_df[master_ledger_df['Description'].str.contains("coffee", na=False)]
            self.assertFalse(coffee_transaction.empty, "Coffee transaction not found in master ledger")
            # Original Actual Amount for coffee was $20. Allowed should be $40 due to "2x"
            self.assertAlmostEqual(coffee_transaction['AllowedAmount'].iloc[0], 40.00, places=2,
            ...
            self.assertGreater(results['analytics']['rent_budget_analysis'].get('total_budget_variance', -9999), -1000) # Check it's calculated
            ...
            logger.error(f"Test 'test_full_analysis_run_v23_fixtures' FAILED. Exception: {e}", exc_info=True)
            # If results were populated before error, log them for debugging
            if results and 'output_paths' in results and 'data_quality_log' in results['output_paths']:
                 logger.error(f"Data quality log available at: {results['output_paths']['data_quality_log']}")
            self.fail(f"Full analysis run failed with v2.3 fixture data: {e}")


# --- Main execution block ---
def main():
    print("Enhanced Shared Expense Analyzer - Institutional Grade (v2.3)")
    print("-" * 80)

    parser = argparse.ArgumentParser(description="Enhanced Shared Expense Analyzer v2.3")
    parser.add_argument("--expense", type=Path, default=Path('Expense_History_20250527.csv'), help="Path to expense history CSV.")
    parser.add_argument("--ledger", type=Path, default=Path('Transaction_Ledger_20250527.csv'), help="Path to transaction ledger CSV.")
    parser.add_argument("--rent", type=Path, default=Path('Rent_Allocation_20250527.csv'), help="Path to rent allocation CSV.")
    parser.add_argument("--rent-hist", type=Path, default=Path('Rent_History_20250527.csv'), help="Path to rent history CSV.")
    ...
    if not np.isclose(config.RYAN_PCT + config.JORDYN_PCT, 1.0):
        print(f"Error: Ryan's percentage ({config.RYAN_PCT}) and Jordyn's percentage ({config.JORDYN_PCT}) do not sum to 1.0.")
    ...
    for file_path, file_name in files_to_check:
        if not file_path.exists():
            print(f"Error: {file_name} file not found at {file_path.resolve()}")
            logger.critical(f"{file_name} file not found: {file_path.resolve()}")
            all_files_found = False
    if not all_files_found:
        print("One or more required data files were not found. Please check paths and try again.")
        return
        
    print(f"Using Configuration: Ryan Pct={config.RYAN_PCT*100}%, Jordyn Pct={config.JORDYN_PCT*100}%, Rent Baseline=${config.RENT_BASELINE:,.2f}")
    print(f"Using files:")
    print(f"  - Expense History: {args.expense.resolve()}")
    print(f"  - Transaction Ledger: {args.ledger.resolve()}")
    print(f"  - Rent Allocation: {args.rent.resolve()}")
    print(f"  - Rent History: {args.rent_hist.resolve()}")
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("ANALYSIS COMPLETE - Version 2.3")
        print("=" * 80 + "\n")
        ...
        for src, details in results.get('data_sources_summary', {}).items():
            print(f"  - {src.replace('_',' ').title()}: {details.get('rows',0)} rows loaded. "
                  f"Date Range: {details.get('date_range',(None,None))[0]} - {details.get('date_range',(None,None))[1]}")
        ...
        if dq_summary and not ("All Clear" in dq_summary and len(dq_summary)==1) :
            for flag, count in dq_summary.items():
                if flag != "All Clear": print(f"  - {flag}: {count}")
        else:
            print("  - All Clear or no specific data quality flags raised.")

        print("\nOutput Files Generated In 'analysis_output' directory:")
        for name, path_str in results.get('output_paths', {}).items():
            print(f"  - {name.replace('_',' ').title()}: {Path(path_str).name}")
        ...
        if dashboard_path: print(f"Interactive dashboard: {Path(dashboard_path).resolve()}")
        if pdf_path: print(f"Executive PDF report: {Path(pdf_path).resolve()}")

    except FileNotFoundError as fnf_err: # Should be caught by pre-check now
        print(f"\nANALYSIS FAILED - File Not Found: {fnf_err}")
